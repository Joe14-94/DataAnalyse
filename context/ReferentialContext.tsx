...
export const ReferentialProvider: React.FC<ReferentialProviderProps> = ({
  ...
  const addAccount = (chartId: string, account: Omit<Account, 'id' | 'createdAt'>) => {
    ...
    onUpdate({
      ...referentials,
      chartOfAccounts: chartsOfAccounts.map((chart) =>
        chart.id === chartId ? { ...chart, accounts: [...chart.accounts, newAccount] } : chart
      )
    });
  };
  ...
  const updateAxisValue = (id: string, updates: Partial<AxisValue>) => {
    onUpdate({
      ...referentials,
      axisValues: axisValues.map((val) => (val.id === id ? { ...val, ...updates } : val))
    });
  };
  ...
  const updateFiscalCalendar = (id: string, updates: Partial<FiscalCalendar>) => {
    onUpdate({
      ...referentials,
      fiscalCalendars: fiscalCalendars.map((cal) => (cal.id === id ? { ...cal, ...updates } : cal))
    });
  };
  ...
  const getCurrentFiscalCalendar = (): FiscalCalendar | null => {
    ...
    return (
      fiscalCalendars.find((cal) => cal.fiscalYear === currentYear) || fiscalCalendars[0] || null
    );
  };
  ...
  const updateMasterDataItem = (id: string, updates: Partial<MasterDataItem>) => {
    onUpdate({
      ...referentials,
      masterData: masterData.map((item) => (item.id === id ? { ...item, ...updates } : item))
    });
  };
  ...
  return <ReferentialContext.Provider value={value}>{children}</ReferentialContext.Provider>;
};
