...
export const DataProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [lastDataExplorerState, setLastDataExplorerState] = useState<DataExplorerState | null>(
    null
  );
  ...
  const [budgetModule, setBudgetModule] = useState<BudgetModule>({
    budgets: [],
    templates: [],
    comments: [],
    notifications: []
  }); // NEW - Budget Module
  const [forecastModule, setForecastModule] = useState<ForecastModule>({
    forecasts: [],
    reconciliationReports: []
  }); // NEW - Forecast Module
  const [pipelineModule, setPipelineModule] = useState<PipelineModule>({
    pipelines: [],
    executionResults: {}
  }); // NEW - Pipeline Module
  ...
  const currentDataset = useMemo(
    () => (datasets || []).find((d) => d.id === currentDatasetId) || null,
    ...
    const init = async () => {
      try {
        ...
        if (dbData) {
          ...
          setBudgetModule(
            dbData.budgetModule || { budgets: [], templates: [], comments: [], notifications: [] }
          ); // NEW - Budget Module
          ...
          if (stored) {
            ...
            setBudgetModule(
              parsed.budgetModule || { budgets: [], templates: [], comments: [], notifications: [] }
            ); // NEW - Budget Module
  ...
  const createDataset = useCallback(
    (name: string, fields: string[], fieldConfigs?: Record<string, FieldConfig>) => {
  ...
  const createDerivedDataset = useCallback(
    (name: string, isTemporal: boolean, config: any, fields: string[], rows: any[]) => {
      const newId = generateId();
      const newDataset: Dataset = {
        id: newId,
        name,
  ...
  const addFieldToDataset = useCallback(
    (datasetId: string, fieldName: string, config?: FieldConfig) => {
      setDatasets((prev) =>
        prev.map((d) => {
  ...
  const updateCalculatedField = useCallback(
    (datasetId: string, fieldId: string, updates: Partial<CalculatedField>) => {
      setDatasets((prev) =>
        prev.map((d) => {
          if (d.id !== datasetId) return d;
          return {
            ...d,
            calculatedFields: (d.calculatedFields || []).map((f) =>
  ...
  const updateDatasetConfigs = useCallback(
    (datasetId: string, configs: Record<string, FieldConfig>) => {
      setDatasets((prev) =>
        prev.map((d) => {
          if (d.id !== datasetId) return d;
          return {
  ...
  const addCalculatedField = useCallback(
    (datasetId: string, field: CalculatedField) => {
      setDatasets((prev) =>
        prev.map((d) => {
          if (d.id !== datasetId) return d;
          return {
  ...
  const addBatch = useCallback(
    (datasetId: string, date: string, rows: any[]) => {
        ...
        if (dataset.enrichmentConfigs && dataset.enrichmentConfigs.length > 0) {
          dataset.enrichmentConfigs.forEach((config) => {
            ...
            const targetBatches = batches
              .filter((b) => b.datasetId === config.targetDatasetId)
            ...
            if (targetBatches.length > 0) {
              ...
              processedRows = processedRows.map((row) => {
                ...
                const val = matchedRow
                  ? config.columnsToAdd.length === 1
                    ? matchedRow[config.columnsToAdd[0]]
                    : config.columnsToAdd.map((col) => matchedRow[col]).join(' | ')
                  : null;
        ...
        while (changedDatasetIds.size > 0 && totalLoops < MAX_LOOPS) {
          ...
          const derivedToUpdate = (datasets || []).filter((d) => {
            ...
            if (d.sourcePivotConfig.isTemporal) {
              ...
              return (
                config?.sources?.some((s: any) => changedDatasetIds.has(s.datasetId)) ||
          ...
          derivedToUpdate.forEach((derivedDataset) => {
            try {
              ...
              if (isTemporal) {
                ...
                const primaryDS = (datasets || []).find(
                  (d) => d.id === tConfig.sources?.[0]?.datasetId
                );
                const dateColumn = primaryDS
                  ? detectDateColumn(primaryDS.fields) || 'Date écriture'
                  : 'Date écriture';
                ...
                if (tConfig.comparisonMode === 'mtd' || tConfig.comparisonMode === 'ytd') {
                  const latestBatch = finalBatches
                    .filter((b) => b.datasetId === primaryDS?.id)
                    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())[0];
                  if (latestBatch) {
                    const latestMonth = new Date(latestBatch.date).getMonth() + 1;
                    activeConfig.comparisonMonth = latestMonth;
                    activeConfig.periodFilter =
                      tConfig.comparisonMode === 'ytd'
                        ? { startMonth: 1, endMonth: latestMonth }
                        : { startMonth: latestMonth, endMonth: latestMonth };
                  }
                }

                const { results } = calculateTemporalComparison(
                  sourceDataMap,
                  activeConfig,
                  dateColumn,
                  false,
                  activeConfig.filters || config.filters || []
                );
                derivedRows = temporalResultToRows(
                  results,
                  activeConfig.groupByFields || config.rowFields || [],
                  activeConfig
                );
              } else if (config) {
                const primarySource = config.sources?.find((s: any) => s.isPrimary) || {
                  datasetId: config.currentDataset?.id,
                  isPrimary: true
                };
                const primaryDS = (datasets || []).find((d) => d.id === primarySource.datasetId);

                if (primaryDS) {
                  const dsBatches = finalBatches
                    .filter((b) => b.datasetId === primaryDS.id)
                    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
              ...
              if (derivedRows.length > 0) {
                ...
                if (
                  !finalBatches.some((b) => b.datasetId === derivedDataset.id && b.date === date)
                ) {
                  finalBatches.push({
                    id: generateId(),
  ...
  const enrichBatchesWithLookup = useCallback(
    (
      datasetId: string,
      ...
      // Get target dataset's latest batch
      const targetBatches = batches
        .filter((b) => b.datasetId === targetDatasetId)
        .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
  ...
  const importBackup = useCallback(async (jsonData: string, keys?: (keyof AppState)[]) => {
    try {
      ...
      if (shouldImport('savedMappings') && parsed.savedMappings)
        setSavedMappings(parsed.savedMappings);
      if (shouldImport('dashboardWidgets') && parsed.dashboardWidgets)
        setDashboardWidgets(parsed.dashboardWidgets);
      if (shouldImport('savedAnalyses') && parsed.savedAnalyses)
        setSavedAnalyses(parsed.savedAnalyses);
      if (shouldImport('lastPivotState') && parsed.lastPivotState)
        setLastPivotState(parsed.lastPivotState);
      if (shouldImport('lastAnalyticsState') && parsed.lastAnalyticsState)
      ...
      if (shouldImport('financeReferentials') && parsed.financeReferentials)
        setFinanceReferentials(parsed.financeReferentials);
      if (shouldImport('budgetModule') && parsed.budgetModule) setBudgetModule(parsed.budgetModule);
      if (shouldImport('forecastModule') && parsed.forecastModule)
        setForecastModule(parsed.forecastModule);
      if (shouldImport('pipelineModule') && parsed.pipelineModule)
        setPipelineModule(parsed.pipelineModule);
      if (shouldImport('uiPrefs') && parsed.uiPrefs) setUiPrefs(parsed.uiPrefs);

      if (shouldImport('currentDatasetId')) {
        if (
          parsed.currentDatasetId &&
          (parsed.datasets || datasets).find((d: Dataset) => d.id === parsed.currentDatasetId)
        ) {
    ...
    >
      <SettingsProvider initialPrefs={uiPrefs} onPrefsChange={setUiPrefs}>
        <ReferentialProvider
          ...
          onUpdate={updateFinanceReferentials}
        >
