...
interface BudgetContextType {
  ...
  addVersion: (
    budgetId: string,
    version: Omit<BudgetVersion, 'id' | 'createdAt' | 'updatedAt'>
  ) => void;
  ...
  compareVersions: (
    ...
    version1Id: string,
    version2Id: string
  ) => BudgetVersionComparison | null;

  // Lignes budgétaires
  addLine: (
    budgetId: string,
    versionId: string,
    line: Omit<BudgetLine, 'id' | 'createdAt' | 'updatedAt'>
  ) => void;
  updateLine: (
    budgetId: string,
    versionId: string,
...
export const BudgetProvider: React.FC<BudgetProviderProps> = ({
  ...
  const updateBudget = (id: string, updates: Partial<Budget>) => {
    onUpdate({
      ...
      budgets: budgets.map((budget) =>
        budget.id === id ? { ...budget, ...updates, updatedAt: Date.now() } : budget
      )
    });
  };
  ...
  const deleteVersion = (budgetId: string, versionId: string) => {
    ...
    updateBudget(budgetId, {
      ...
      activeVersionId:
        budget.activeVersionId === versionId
  ...
  const resolveComment = (commentId: string) => {
    onUpdate({
      ...budgetModule,
      comments: comments.map((c) => (c.id === commentId ? { ...c, isResolved: true } : c))
    });
  };

  const getCommentsByBudget = (budgetId: string, versionId?: string): BudgetComment[] => {
    return comments.filter(
      (c) => c.budgetId === budgetId && (!versionId || c.versionId === versionId)
    );
  };
