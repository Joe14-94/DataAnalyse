...
interface ForecastContextType {
    ...
    // Versions
    addVersion: (forecastId: string, version: Omit<ForecastVersion, 'id' | 'createdAt' | 'updatedAt'>) => void;
    ...
    // Lignes
    addLine: (forecastId: string, versionId: string, line: Omit<ForecastLine, 'id' | 'createdAt' | 'updatedAt'>) => void;
    updateLine: (forecastId: string, versionId: string, lineId: string, updates: Partial<ForecastLine>) => void;
    deleteLine: (forecastId: string, versionId: string, lineId: string) => void;
    updateLineValue: (forecastId: string, versionId: string, lineId: string, periodId: string, value: number) => void;

    // Copy actual values
    copyActualToForecast: (forecastId: string, versionId: string, actualData: { [accountCode: string]: { [period: string]: number } }) => void;
    ...
    updateDriverValue: (forecastId: string, driverId: string, periodId: string, value: number, isHistorical: boolean) => void;
    ...
    // ML Predictions
    generateMLPredictions: (forecastId: string, versionId: string, lineId: string, lookbackMonths: number) => void;
    detectSeasonality: (values: number[]) => { hasSeason: boolean; period?: number };
    calculateTrend: (values: number[]) => { slope: number; intercept: number; trend: 'increasing' | 'decreasing' | 'stable' };

    // Reconciliation
    reconciliationReports: ForecastReconciliationReport[];
    createReconciliationReport: (forecastId: string, versionId: string, actualData: { [accountCode: string]: { [period: string]: number } }) => void;
...
export const ForecastProvider: React.FC<ForecastProviderProps> = ({
    ...
    const updateForecast = (id: string, updates: Partial<Forecast>) => {
        onUpdate({
            ...
            forecasts: forecasts.map(forecast =>
                forecast.id === id
                    ? { ...forecast, ...updates, updatedAt: Date.now() }
                    : forecast
            )
        });
    };
    ...
    // --- VERSIONS ---
    const addVersion = (forecastId: string, version: Omit<ForecastVersion, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    const updateVersion = (forecastId: string, versionId: string, updates: Partial<ForecastVersion>) => {
        const forecast = getForecast(forecastId);
        if (!forecast) return;
    ...
    const deleteVersion = (forecastId: string, versionId: string) => {
        ...
        updateForecast(forecastId, {
            ...
            activeVersionId: forecast.activeVersionId === versionId
    ...
    // --- LIGNES ---
    const addLine = (forecastId: string, versionId: string, line: Omit<ForecastLine, 'id' | 'createdAt' | 'updatedAt'>) => {
        const forecast = getForecast(forecastId);
        if (!forecast) return;
    ...
    const updateLine = (forecastId: string, versionId: string, lineId: string, updates: Partial<ForecastLine>) => {
        const forecast = getForecast(forecastId);
        if (!forecast) return;
    ...
    const updateLineValue = (forecastId: string, versionId: string, lineId: string, periodId: string, value: number) => {
        const forecast = getForecast(forecastId);
        if (!forecast) return;
    ...
    const updateDriverValue = (forecastId: string, driverId: string, periodId: string, value: number, isHistorical: boolean) => {
        const forecast = getForecast(forecastId);
        if (!forecast) return;
    ...
    const calculateTrend = (values: number[]): { slope: number; intercept: number; trend: 'increasing' | 'decreasing' | 'stable' } => {
    ...
    const generateMLPredictions = (forecastId: string, versionId: string, lineId: string, lookbackMonths: number) => {
        const forecast = getForecast(forecastId);
        if (!forecast) return;
    ...
    const createReconciliationReport = (
        ...
        version.lines.forEach(line => {
            ...
            const periods = new Set([
                ...Object.keys(line.forecastValues),
        ...
        const mape = variances.length > 0
            ? variances.reduce((sum, v) => sum + Math.abs(v.variancePercent), 0) / variances.length
            : 0;

        // Calculate RMSE (Root Mean Square Error)
        const rmse = variances.length > 0
            ? Math.sqrt(variances.reduce((sum, v) => sum + Math.pow(v.variance, 2), 0) / variances.length)
            : 0;
        ...
        const report: ForecastReconciliationReport = {
            ...
            periodEnd: new Date(new Date(version.referenceDate).setMonth(new Date(version.referenceDate).getMonth() + 12)).toISOString().split('T')[0],
            variances,
            totalForecast,
            totalActual,
            totalVariance: totalActual - totalForecast,
            totalVariancePercent: totalForecast !== 0 ? ((totalActual - totalForecast) / totalForecast) * 100 : 0,
            mape,
            rmse,
            recommendations,
            createdAt: Date.now()
