...
export const SunburstD3: React.FC<SunburstD3Props> = ({
  ...
  useEffect(() => {
    ...
    const colorScale =
      colors.length > 0 ? d3.scaleOrdinal(colors) : d3.scaleOrdinal(d3.schemeCategory10);
    ...
    const path = svg
      ...
      .on('mouseover', (event, d) => {
        ...
        const value = d.value?.toLocaleString('fr-FR', {
          minimumFractionDigits: 0,
          maximumFractionDigits: 2
        });
        const percent = (((d.value || 0) / totalValue) * 100).toLocaleString('fr-FR', {
