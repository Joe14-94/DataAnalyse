    ...
    return (
      ...
      >
        <div
          ...
          className={`${bgColor} rounded-lg ${widthClass} ${borderClass} ${isEditMode ? 'ring-2 ring-brand-100 border-brand-300 cursor-move' : ''} shadow-sm p-4 flex flex-col ${heightClass} relative group transition-all`}
        >
          {!isEditMode && (
            <div className="absolute top-2 right-2 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
              <div className="relative">
                <button
                  onClick={() =>
                    setOpenMenuWidgetId(openMenuWidgetId === widget.id ? null : widget.id)
                  }
                ...
                {openMenuWidgetId === widget.id && (
                  <div className="absolute right-0 mt-1 w-40 bg-white rounded-lg shadow-lg border border-slate-200 py-1 z-20 animate-in fade-in zoom-in-95 duration-100">
                    ...
                    <button
                      onClick={() => handleExportImage(widget.id, widget.title)}
                      className="w-full text-left px-3 py-2 text-xs hover:bg-slate-50 flex items-center gap-2 text-slate-700"
                    >
                      <ImageIcon className="w-3 h-3" /> Image (.png)
                    </button>
                    <button
                      onClick={() => handleExportCSV(widgetData, widget.title)}
                      className="w-full text-left px-3 py-2 text-xs hover:bg-slate-50 flex items-center gap-2 text-slate-700"
                    >
          ...
          {isEditMode && (
            <div className="absolute top-2 right-2 flex gap-1 bg-white/90 p-1 rounded shadow-sm z-10 border border-slate-200 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button
                onClick={() => duplicateDashboardWidget(widget.id)}
          ...
          <h3
            className="text-sm font-bold text-slate-500 mb-1.5 uppercase tracking-wider truncate"
            title={widget.title}
          >
            {widget.title}
          </h3>
