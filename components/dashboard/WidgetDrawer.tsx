...
const LivePreview: React.FC<{ widget: DashboardWidget; globalDateRange: any }> = ({
  widget,
  globalDateRange
}) => {
  ...
  const bgColor =
    isText && widget.config.textStyle?.color === 'primary'
  ...
  return (
    <div
      className={`w-full h-full rounded-lg ${borderClass} ${widthClass} shadow-sm p-4 flex flex-col ${bgColor} relative`}
    >
      <h3 className="text-xs font-bold text-slate-500 mb-2 uppercase tracking-wider truncate">
    ...
    <div className="fixed inset-0 z-50 flex bg-slate-900/10 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="flex-1 flex flex-col overflow-hidden relative border-r border-slate-200">
        ...
        <div className="flex-1 overflow-auto flex items-center justify-center p-8">
          <div
            className={`w-full transition-all duration-300 ${tempWidget.size === 'full' ? 'max-w-full' : 'max-w-[500px]'}`}
            style={{ height: tempWidget.height === 'xl' ? '500px' : '256px' }}
          >
                ...
                <>
                  <div>
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        setTempWidget({ ...tempWidget, size: e.target.value as WidgetSize })
                      }
                    >
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        setTempWidget({ ...tempWidget, height: e.target.value as WidgetHeight })
                      }
                    >
                  ...
                  <Select
                    value={
                      tempWidget.config?.source?.mode === 'temporal'
                        ? 'temporal'
                        : tempWidget.config?.source?.datasetId || ''
                    }
                    onChange={(e) => {
                      ...
                      setTempWidget({
                        ...
                        config: {
                          ...tempWidget.config!,
                          source: { datasetId: val, mode: 'latest' }
                        }
                      });
                    }}
                  >
                    ...
                    {tempWidget.config?.source?.mode === 'temporal' && (
                      <option value="temporal">
                        ...
                        {datasets.find((d) => d.id === tempWidget.config?.source?.datasetId)
                          ?.name || 'Multi-sources'}
                        )
                      </option>
                    )}
                ...
                {tempWidget.config?.pivotChart && (
                  <div className="flex items-center gap-3 p-3 bg-brand-50 border border-brand-100 rounded-lg">
                    <input
                      ...
                      onChange={(e) =>
                        updateConfig({ updateMode: e.target.checked ? 'latest' : 'fixed' })
                      }
                      className="w-4 h-4 text-brand-600 rounded border-slate-300 focus:ring-brand-500"
                    />
                    <label
                      htmlFor="auto-update-mode"
                      className="text-xs font-bold text-brand-800 cursor-pointer select-none"
                    >
                      {tempWidget.config?.source?.mode === 'temporal'
                  ...
                  {BORDER_COLORS.map((c) => (
                    <button
                      ...
                      onClick={() =>
                        setTempWidget({
                          ...tempWidget,
                          style: { ...tempWidget.style, borderColor: c.class }
                        })
                      }
                ...
                {tempWidget.config?.colorMode === 'multi' && (
                  <div>
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        updateConfig({ colorPalette: e.target.value as ColorPalette })
                      }
                    >
