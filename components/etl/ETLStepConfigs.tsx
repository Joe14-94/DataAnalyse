...
import { Plus, ArrowLeftRight, Trash2, X } from 'lucide-react';
...
interface FilterConfigProps {
    ...
    onUpdate: (config: FilterConfigProps['config']) => void;
}

export const FilterConfig: React.FC<FilterConfigProps> = ({ config, availableColumns, onUpdate }) => {
    const addCondition = () => {
        onUpdate({
            ...config,
    ...
    const removeCondition = (index: number) => {
        onUpdate({ ...config, conditions: config.conditions.filter((_, i) => i !== index) });
    };
...
interface CalculateConfigProps {
    config: { newColumn: string; formula: string };
    availableColumns: string[];
    onUpdate: (config: CalculateConfigProps['config']) => void;
}

export const CalculateConfig: React.FC<CalculateConfigProps> = ({ config, availableColumns, onUpdate }) => (
    <div className="space-y-3">
        <div>
...
interface SelectConfigProps {
    ...
    availableColumns: string[];
    onUpdate: (config: SelectConfigProps['config']) => void;
}

export const SelectConfig: React.FC<SelectConfigProps> = ({ config, availableColumns, onUpdate }) => {
        ...
        <div className="space-y-3">
            <div className="flex items-center gap-4 mb-2">
                <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" checked={!config.exclude} onChange={() => onUpdate({ ...config, exclude: false })} />
                    <span className="text-sm font-medium">Conserver</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" checked={config.exclude} onChange={() => onUpdate({ ...config, exclude: true })} />
...
interface AggregateConfigProps {
    config: { groupBy: string[]; aggregations: { field: string; operation: ETLAggregationType; alias?: string }[] };
    availableColumns: string[];
    onUpdate: (config: AggregateConfigProps['config']) => void;
}

export const AggregateConfig: React.FC<AggregateConfigProps> = ({ config, availableColumns, onUpdate }) => {
    const addAggregation = () => {
        onUpdate({
            ...config,
            aggregations: [...config.aggregations, { field: availableColumns[0], operation: 'sum' as ETLAggregationType }]
        });
    };
                    ...
                    <div key={idx} className="flex gap-2 mb-2 items-center">
                        <select
                            ...
                            onChange={(e) => {
                                ...
                                newAggs[idx] = { ...newAggs[idx], field: e.target.value };
                        ...
                        <select
                            ...
                            onChange={(e) => {
                                const newAggs = [...config.aggregations];
                                newAggs[idx] = { ...newAggs[idx], operation: e.target.value as ETLAggregationType };
                                onUpdate({ ...config, aggregations: newAggs });
                            }}
...
interface SortConfigProps {
    ...
    availableColumns: string[];
    onUpdate: (config: SortConfigProps['config']) => void;
}

export const SortConfig: React.FC<SortConfigProps> = ({ config, availableColumns, onUpdate }) => {
    const addSort = () => {
        onUpdate({ ...config, fields: [...config.fields, { field: availableColumns[0], direction: 'asc' }] });
    };
            ...
            {config.fields.map((s, idx) => (
                <div key={idx} className="flex gap-2 mb-1">
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            const newFields = [...config.fields];
                            newFields[idx] = { ...newFields[idx], direction: e.target.value as 'asc' | 'desc' };
                            onUpdate({ ...config, fields: newFields });
                        }}
                        className="text-xs p-1 border border-slate-300 rounded"
                    >
...
interface RenameConfigProps {
    ...
    availableColumns: string[];
    onUpdate: (config: RenameConfigProps['config']) => void;
}

export const RenameConfig: React.FC<RenameConfigProps> = ({ config, availableColumns, onUpdate }) => {
    const addMapping = () => {
        onUpdate({ ...config, mappings: [...config.mappings, { oldName: availableColumns[0], newName: '' }] });
    };
                ...
                <div key={idx} className="flex gap-2 mb-1">
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            newMappings[idx] = { ...newMappings[idx], oldName: e.target.value };
                    ...
                    <input
                        ...
                        onChange={(e) => {
                            const newMappings = [...config.mappings];
                            newMappings[idx] = { ...newMappings[idx], newName: e.target.value };
                            onUpdate({ ...config, mappings: newMappings });
                        }}
                        className="text-xs p-1 border border-slate-300 rounded flex-1"
                    />
...
interface SplitConfigProps {
    ...
    availableColumns: string[];
    onUpdate: (config: SplitConfigProps['config']) => void;
}

export const SplitConfig: React.FC<SplitConfigProps> = ({ config, availableColumns, onUpdate }) => (
    <div className="space-y-3">
        <div>
...
interface MergeConfigProps {
    ...
    availableColumns: string[];
    onUpdate: (config: MergeConfigProps['config']) => void;
}

export const MergeConfig: React.FC<MergeConfigProps> = ({ config, availableColumns, onUpdate }) => (
    <div className="space-y-3">
        <div>
...
interface PivotConfigProps {
    ...
    availableColumns: string[];
    onUpdate: (config: PivotConfigProps['config']) => void;
}

export const PivotConfig: React.FC<PivotConfigProps> = ({ config, availableColumns, onUpdate }) => (
    <div className="grid grid-cols-2 gap-4">
        <div>
...
interface JoinConfigProps {
    ...
    availableColumns: string[];
    datasets: Dataset[];
    onUpdate: (config: JoinConfigProps['config']) => void;
}

export const JoinConfig: React.FC<JoinConfigProps> = ({ config, availableColumns, datasets, onUpdate }) => {
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select value={config.rightKey} onChange={(e) => onUpdate({ ...config, rightKey: e.target.value })} className="w-full text-xs p-2 border border-slate-300 rounded">
                        <option value="">-- Sélectionner --</option>
                        {rightDS?.fields.map(f => <option key={f} value={f}>{f}</option>)}
                    </select>
                </div>
            </div>
        </div>
...
interface UnpivotConfigProps {
    ...
    availableColumns: string[];
    onUpdate: (config: UnpivotConfigProps['config']) => void;
}

export const UnpivotConfig: React.FC<UnpivotConfigProps> = ({ config, availableColumns, onUpdate }) => (
    <div className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
            <div>
...
interface UnionConfigProps {
    ...
    datasets: Dataset[];
    onUpdate: (config: UnionConfigProps['config']) => void;
}

export const UnionConfig: React.FC<UnionConfigProps> = ({ config, datasets, onUpdate }) => (
    <div>
...
export const StepConfiguration: React.FC<StepConfigurationProps> = ({ step, availableColumns, datasets, onUpdate }) => {
    switch (step.type) {
        case 'filter':
