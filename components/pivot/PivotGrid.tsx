...
interface PivotGridProps {
  ...
  setColumnLabels: (v: React.SetStateAction<Record<string, string>>) => void;
  ...
  setColumnWidths: (v: React.SetStateAction<Record<string, number>>) => void;
  styleRules: PivotStyleRule[];
  conditionalRules: ConditionalFormattingRule[];
  onRemoveField?: (zone: 'row' | 'col' | 'val', field: string) => void;
}
...
export const PivotGrid: React.FC<PivotGridProps> = (props) => {
  ...
  const getMetricInfoFromCol = (col: string) => {
    ...
    if (directMetric)
      return { colLabel: 'ALL', metricLabel: baseCol, metric: directMetric, isDiff, isPct };
  ...
  return (
    <div
      id="pivot-export-container"
      className="flex-1 bg-white rounded-lg border border-slate-200 shadow-sm flex flex-col min-w-0 overflow-hidden relative"
    >
      {isCalculating && (
      ...
      {isTemporalMode && temporalResults.length > 0 && temporalConfig ? (
        <div ref={parentRef} className="flex-1 overflow-auto custom-scrollbar w-full relative">
          <table
            className="min-w-max divide-y divide-slate-200 border-collapse"
            ...
            <thead className="sticky top-0 z-30 shadow-sm">
              <tr className="bg-slate-50">
                {(rowFields || []).map((field: string, idx: number) => {
                  ...
                  const left = rowFields
                    .slice(0, idx)
                    .reduce((acc, f) => acc + (columnWidths[`group_${f}`] || 150), 0);
                  ...
                  return (
                    ...
                    >
                        ...
                        <span className="truncate flex-1">
                          {isEditing ? (
                            <input
                              ...
                              onChange={(e) =>
                                setColumnLabels((prev: any) => ({
                                  ...prev,
                                  [`group_${field}`]: e.target.value
                                }))
                              }
                      ...
                      <div
                        className="absolute -right-1 top-0 bottom-0 w-3 cursor-col-resize hover:bg-brand-400/40 z-30 transition-colors"
                        onMouseDown={(e) => onResizeStart(e, widthId, 150)}
                      />
                    </th>
                  );
                })}
                  ...
                  return (
                    <React.Fragment key={`m-${mIdx}`}>
                      {(temporalConfig?.sources || []).map((source: any) => {
                        ...
                        const headerStyle = getCellFormatting(
                          [],
                          colKey,
                          undefined,
                          mLabel,
                        ...
                        const displayLabel =
                          metrics.length > 1 ? `${source.label} - ${mLabel}` : source.label;
                        ...
                        return (
                          <React.Fragment key={source.id}>
                            ...
                            >
                              ...
                              <div
                                className="absolute -right-1 top-0 bottom-0 w-3 cursor-col-resize hover:bg-brand-400/40 z-30 transition-colors"
                                onMouseDown={(e) => onResizeStart(e, colKey, 120)}
                              />
                            </th>
                            {showVariations && source.id !== temporalConfig.referenceSourceId && (
                              <th
                                className="px-2 py-1.5 text-right text-xs font-bold uppercase border-b border-r border-slate-200 bg-purple-50 text-purple-700"
              ...
              {temporalResults.map((result) => {
                ...
                return (
                  <tr
                    ...
                    className={
                      isSubtotal
                        ? `bg-slate-50 font-bold border-t border-slate-200`
                    ...
                    {(() => {
                      ...
                      return Array.from({ length: numFields }, (_, gIdx) => {
                        ...
                        const left = rowFields
                          .slice(0, gIdx)
                          .reduce((acc, f) => acc + (columnWidths[`group_${f}`] || 150), 0);
                        const rowStyle = getCellFormatting(
                          result.groupLabel.split('\x1F'),
                          '',
                          undefined,
                          '',
                          isSubtotal ? 'subtotal' : 'data'
                        );
                        ...
                        return (
                          <td
                            ...
                            colSpan={
                              isSubtotal && gIdx === subtotalLevel ? numFields - subtotalLevel : 1
                            }
                            onClick={() =>
                              handleDrilldown(
                                result.groupLabel.split('\x1F').slice(0, gIdx + 1),
                                '',
                                undefined,
                                ''
                              )
                      ...
                      return (
                        <React.Fragment key={`m-data-${mIdx}`}>
                          {(temporalConfig?.sources || []).map((source: any) => {
                            ...
                            const delta = result.deltas[source.id]?.[mLabel] || {
                              value: 0,
                              percentage: 0
                            };
                            const colKey = `${source.id}_${mLabel}`;
                            const width = columnWidths[colKey] || 120;
                            const customStyle = getCellFormatting(
                              result.groupLabel.split('\x1F'),
                              colKey,
                              value,
                              mLabel,
                            ...
                            const isSelected =
                              isSelectionMode &&
                              isItemSelected(result.groupLabel.split('\x1F'), source.label);
                              ...
                              <React.Fragment key={source.id}>
                                ...
                                {showVariations &&
                                  source.id !== temporalConfig.referenceSourceId && (
                                    <td
                                      className={`px-2 py-1 text-xs text-right border-r tabular-nums font-bold overflow-hidden truncate ${delta.value > 0 ? 'text-green-600' : delta.value < 0 ? 'text-red-600' : 'text-slate-400'}`}
                                      style={{ width: 60, minWidth: 60, maxWidth: 60 }}
                                    >
                                      {temporalConfig.deltaFormat === 'percentage'
                                        ? delta.percentage !== 0
                                          ...
                                          : '-'
                                        : delta.value !== 0
                                          ? formatOutput(delta.value, metric)
                                          : '-'}
                                    </td>
                                  )}
                              </React.Fragment>
        ...
        <div
          ref={parentRef}
          className="flex-1 overflow-auto custom-scrollbar flex flex-col w-full relative"
        >
          <div style={{ height: `${rowVirtualizer.getTotalSize()}px`, position: 'relative' }}>
            <table
              className="min-w-max divide-y divide-slate-200 border-collapse absolute top-0 left-0"
              style={{ tableLayout: 'fixed' }}
            >
              <thead className="sticky top-0 z-30 shadow-sm">
                <tr className="bg-slate-50">
                  {rowFields.map((field, idx) => {
                    ...
                    const left = rowFields
                      .slice(0, idx)
                      .reduce((acc, f) => acc + (columnWidths[`row_${f}`] || 150), 0);
                    const calcField = primaryDataset?.calculatedFields?.find(
                      (cf) => cf.name === field
                    );
                    const headerStyle = getCellFormatting([field], '', undefined, '', 'data');
                  ...
                  {(pivotData?.colHeaders || []).map((col: string) => {
                    const { colLabel, metricLabel, metric, isDiff, isPct } =
                      getMetricInfoFromCol(col);
                    let displayLabel = isDiff
                      ? 'Var.'
                      : isPct
                        ? '%'
                        : formatDateLabelForDisplay(colLabel);
                    if (metricLabel && !isDiff && !isPct && colLabel === 'ALL')
                      displayLabel = metricLabel;
                    else if (metricLabel && !isDiff && !isPct)
                      displayLabel = `${displayLabel} - ${metricLabel}`;

                    const width = columnWidths[col] || 120;
                    const calcField = metric
                      ? primaryDataset?.calculatedFields?.find((cf) => cf.name === metric.field)
                      : null;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          className="absolute -right-1 top-0 bottom-0 w-3 cursor-col-resize hover:bg-brand-400/40 z-30 transition-colors"
                          onMouseDown={(e) => onResizeStart(e, col, 120)}
                        />
                      </th>
                    );
                  })}
                  {showTotalCol && (
                    ...
                    >
                      <div className="flex items-center justify-end relative">
                        ...
                        <div
                          className="absolute -right-2 top-0 bottom-0 w-4 cursor-col-resize hover:bg-brand-400/40 z-30 transition-colors"
                          onMouseDown={(e) => onResizeStart(e, 'Grand Total', 150)}
                        />
                      </div>
                    </th>
                  )}
              ...
              <tbody className="bg-white divide-y divide-slate-200">
                {paddingTop > 0 && (
                  <tr>
                    <td style={{ height: `${paddingTop}px` }} colSpan={totalColumns} />
                  </tr>
                )}
                {virtualItems.map((virtualRow: any) => {
                  ...
                  return (
                    <tr
                      ...
                      className={`${row.type === 'subtotal' ? 'bg-slate-50 font-bold' : 'hover:bg-brand-50/30'}`}
                    >
                      {(rowFields || []).map((field, cIdx) => {
                        ...
                        return (
                          <td
                            ...
                            onClick={() =>
                              handleDrilldown(row.keys.slice(0, cIdx + 1), '', undefined, '')
                            }
                          >
                            {row.keys[cIdx]}
                          </td>
                        ...
                        >
                          {typeof row.rowTotal === 'object' ? (
                            <div className="flex flex-col gap-0.5">
                              {Object.entries(row.rowTotal).map(([label, v], idx) => {
                                const metric = metrics.find(
                                  (m) => (m.label || `${m.field} (${m.aggType})`) === label
                                );
                                const metricStyle = getCellFormatting(
                                  row.keys,
                                  'Total',
                                  v,
                                  label,
                            ...
                            <span className="text-xs font-bold text-slate-800">
                              {formatOutput(row.rowTotal, metrics[0])}
                            </span>
                          )}
                        </td>
                      )}
                ...
                {paddingBottom > 0 && (
                  <tr>
                    <td style={{ height: `${paddingBottom}px` }} colSpan={totalColumns} />
                  </tr>
                )}
              </tbody>
            </table>
