...
export const PivotFooter: React.FC<PivotFooterProps> = ({
  ...
  if (temporalColTotals && temporalConfig) {
    return (
      <div
        ref={footerRef}
        className="border-t-2 border-slate-300 bg-slate-100 shadow-inner overflow-x-hidden flex-shrink-0"
      >
        <table
                ...
                return (
                  <React.Fragment key={mLabel}>
                    {(temporalConfig?.sources || []).map((source: any) => {
                      ...
                      const customStyle = getCellStyle(
                        [],
                        colKey,
                        val,
                        mLabel,
                      ...
                      const referenceTotal =
                        temporalColTotals[temporalConfig.referenceSourceId]?.[mLabel] || 0;
                      const deltaValue = val - referenceTotal;
                      const deltaPercentage =
                        referenceTotal !== 0
                          ? (deltaValue / referenceTotal) * 100
                          : val !== 0
                            ? 100
                            : 0;
                            ...
                            >
                              {temporalConfig.deltaFormat === 'percentage'
                                ? deltaPercentage !== 0
                                  ...
                                  : '-'
                                : deltaValue !== 0
                                  ? formatOutput(deltaValue, metric)
                                  : '-'}
                            </td>
                          )}
                        </React.Fragment>
        ...
        <tbody className="font-bold">
          <tr>
            ...
            {pivotData?.colHeaders.map((col: string) => {
              ...
              const metricLabel =
                metric?.label || (metric?.field ? `${metric.field} (${metric.aggType})` : '');
                ...
                style={{
                  ...getCellFormatting(
                    ...
                    typeof pivotData.grandTotal === 'object'
                      ? Object.values(pivotData.grandTotal)[0]
                ...
                {typeof pivotData.grandTotal === 'object' ? (
                  <div className="flex flex-col gap-0.5">
                    {Object.entries(pivotData.grandTotal).map(([label, v], idx) => {
                      ...
                      const metricStyle = getCellStyle(
                        [],
                        'Total',
                        v,
                        label,
                  ...
                  <span className="text-xs text-black">
                    {formatOutput(pivotData.grandTotal, metrics[0])}
                  </span>
                )}
              </td>
            )}
