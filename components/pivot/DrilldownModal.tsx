    ...
    <Modal
      ...
      title={
        <div>
          ...
          <p className="text-xs text-txt-secondary font-normal">
            {rows.length} ligne{rows.length > 1 ? 's' : ''} de détail
          </p>
        </div>
      }
      footer={
      ...
      <div className="border border-border-default rounded-lg overflow-hidden">
        <table className="w-full text-xs">
          ...
          <tbody className="divide-y divide-border-default">
            {rows.map((row, rowIdx) => (
              <tr key={rowIdx} className="hover:bg-brand-50/30 transition-colors">
                {fields.map((field, colIdx) => {
                  ...
                  return (
                    <td key={colIdx} className="px-3 py-2 text-txt-secondary">
                      {displayValue}
                    </td>
                  );
                })}
