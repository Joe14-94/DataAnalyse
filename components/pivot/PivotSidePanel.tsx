   ...
   return (
      <div ref={panelRef} className="w-72 flex-shrink-0 flex flex-col gap-2 min-w-0 h-full overflow-hidden">
         ...
         <div className="p-1.5 bg-slate-50 rounded border border-slate-200 text-xs flex flex-col gap-1">
            ...
            {(isTemporalMode || colFields.length > 0 || metrics.length > 1) && <Checkbox checked={showVariations} onChange={() => setShowVariations(!showVariations)} label="Comparaisons" className="text-brand-700 font-bold" />}
         </div>
      </div>
   );
};
