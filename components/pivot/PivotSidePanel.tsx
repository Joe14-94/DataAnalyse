  ...
  return (
    ...
    >
      <div className="flex items-center gap-1 overflow-hidden flex-1">
        <GripVertical
          className={`w-2.5 h-2.5 flex-shrink-0 ${disabled ? 'text-slate-300' : 'text-slate-400'}`}
        />
...
export const PivotSidePanel: React.FC<PivotSidePanelProps> = (props) => {
  ...
  return (
    <div
      ref={panelRef}
      className="w-72 flex-shrink-0 flex flex-col gap-2 min-w-0 h-full overflow-hidden"
    >
      {/* 1. DATA SOURCES STACK */}
      <div
        className="bg-white rounded-lg border border-slate-200 shadow-sm flex flex-col overflow-hidden min-h-[40px]"
        style={{ maxHeight: isDataSourcesPanelCollapsed ? '40px' : '220px' }}
      >
            ...
            {sources.length === 0 ? (
              <div className="text-center p-6 border-2 border-dashed border-brand-300 rounded-lg bg-gradient-to-br from-brand-50 to-indigo-50">
                ...
                <button
                  ...
                  className="w-full py-2 bg-brand-600 text-white text-xs font-bold rounded-lg hover:bg-brand-700"
                >
              ...
              <div className="space-y-2">
                {sources.map((src) => {
                  ...
                  const srcColorClasses =
                    (SOURCE_COLOR_CLASSES as any)[src.color] || SOURCE_COLOR_CLASSES.blue;
                  return (
                    <div
                      key={src.id}
                      className={`relative pl-2 border-l-2 ${srcColorClasses.border} ${srcColorClasses.bg} rounded-r-lg p-2 group`}
                    >
                      <div className="flex justify-between items-center mb-1">
                        <div
                          className={`text-xs font-bold ${srcColorClasses.text} flex items-center gap-1.5 overflow-hidden`}
                        >
                          {src.isPrimary ? (
                            <Database className="w-3 h-3 flex-shrink-0" />
                          ) : (
                            <Plus className="w-3 h-3 flex-shrink-0 rotate-45" />
                          )}
                      ...
                      {src.isPrimary ? (
                        ...
                        <div className="text-xs text-slate-500 truncate">
                          Jointure: {src.joinConfig?.primaryKey} = {src.joinConfig?.secondaryKey}
                        </div>
                      )}
                    </div>
                  );
                ...
                <button
                  onClick={startAddSource}
                  className="w-full py-1 border-2 border-dashed border-slate-300 rounded text-slate-500 hover:text-brand-600 hover:border-brand-400 text-xs font-bold flex items-center justify-center gap-1"
                >
        ...
        <div className="p-2 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-green-200">
          <div className="flex items-center justify-between gap-2 mb-1.5">
            <h3 className="text-xs font-bold text-slate-800 flex items-center gap-2">
              <Table2 className="w-3 h-3 text-green-600" /> Champs
            </h3>
            <button
              ...
              return (
                <div key={group.id} className="mb-2">
                  <button
                    ...
                    className={`w-full flex items-center gap-1 text-xs font-bold px-1.5 py-1 rounded transition-colors ${(SOURCE_COLOR_CLASSES as any)[group.color]?.text || 'text-slate-600'} ${(SOURCE_COLOR_CLASSES as any)[group.color]?.bg || 'bg-slate-100'}`}
                  >
                      ...
                      {filteredFields.map((f: string) => {
                        ...
                        return (
                          <FieldChip
                            ...
                            onEdit={
                              calcField && openEditCalcModal
                                ? () => openEditCalcModal(calcField)
                                : undefined
                            }
                            onDelete={
                              calcField && removeCalculatedField
                                ? () => {
      ...
      >
        ...
        <div
          ...
          style={dropZonesHeight ? { height: `${dropZonesHeight}px` } : { flex: 1 }}
        >
          <div
            onDragOver={handleDragOver}
            onDrop={(e) => handleDrop(e, 'filter')}
            className={`bg-white rounded border-2 border-dashed p-1 overflow-auto custom-scrollbar ${draggedField ? 'border-brand-300 bg-brand-50/30' : 'border-slate-200'}`}
          >
            <div className="text-xs font-bold text-slate-400 uppercase mb-1 flex items-center gap-1 sticky top-0 bg-white/90 backdrop-blur-sm z-10">
              <Filter className="w-2 h-2" /> Filtres
            </div>
            <div className="space-y-2">
              {filters.map((f, idx) => {
                const calcField = primaryDataset?.calculatedFields?.find(
          ...
          >
            <div className="text-xs font-bold text-slate-400 uppercase mb-1 flex items-center gap-1 sticky top-0 bg-white/90 backdrop-blur-sm z-10">
              <Table2 className="w-2 h-2" /> Colonnes
            </div>
            <div className="space-y-1">
              ...
              {isColFieldDate && (
                <select
                  className="w-full text-xs border-slate-200 rounded bg-slate-50 p-0.5"
                  value={colGrouping}
                  onChange={(e) => setColGrouping(e.target.value as any)}
                >
                  <option value="none">Brut</option>
                  <option value="year">Année</option>
                  <option value="quarter">T.</option>
        ...
        <div className="grid grid-cols-2 gap-2 flex-1 min-h-0">
          ...
          >
            <div className="text-xs font-bold text-slate-400 uppercase mb-1 flex items-center gap-1 sticky top-0 bg-white/90 backdrop-blur-sm z-10">
              <Layers className="w-2 h-2" /> Lignes
            </div>
            {rowFields.map((f) => {
              const calcField = primaryDataset?.calculatedFields?.find((cf) => cf.name === f);
              return (
                <FieldChip
                  ...
                  isCalculated={!!calcField}
                  onEdit={
                    calcField && openEditCalcModal ? () => openEditCalcModal(calcField) : undefined
                  }
                />
              );
            })}
            ...
            <div className="text-xs font-bold text-slate-400 uppercase mb-1 flex items-center justify-between sticky top-0 bg-white/90 backdrop-blur-sm z-10">
              ...
              {openCalcModal && (
                <button
                  ...
                  className="p-0.5 hover:bg-indigo-50 text-indigo-500 rounded transition-colors"
      ...
      <div className="p-1.5 bg-slate-50 rounded border border-slate-200 text-xs flex flex-col gap-1">
        ...
        <Checkbox
          ...
          label="Total général"
        />
        {(isTemporalMode || colFields.length > 0 || metrics.length > 1) && (
          <Checkbox
