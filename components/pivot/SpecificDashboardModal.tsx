    ...
    return (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4 lg:p-8 animate-in fade-in duration-300">
            <div className="bg-white rounded-2xl shadow-2xl w-[90vw] h-[90vh] overflow-hidden flex flex-col border border-slate-200">
                {/* Header */}
                <div className="p-4 border-b border-slate-200 flex justify-between items-center bg-blue-50 flex-shrink-0">
                    ...
                    <div className="w-full md:w-80 border-r border-slate-200 flex flex-col bg-slate-50">
                        <div className="p-4 border-b border-slate-200 bg-white">
                            <Button
                                ...
                                className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 shadow-sm flex items-center justify-center gap-2"
                            >
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
                            ...
                            {items.length === 0 ? (
                                ...
                                items.map((item, idx) => (
                                    <div key={item.id} className="bg-white border border-slate-200 rounded-lg p-3 shadow-sm hover:border-blue-300 transition-all group relative">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                {editingId === item.id ? (
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <button onClick={() => startEditing(item)} className="opacity-0 group-hover:opacity-100 text-slate-400 hover:text-blue-500 transition-opacity"><Edit2 className="w-2.5 h-2.5" /></button>
                                                    </div>
                                                )}
                                        ...
                                        <div className="text-lg font-black text-blue-600 font-mono mt-1">{item.value}</div>
                                        ...
                                        <div className="absolute -left-3 top-1/2 -translate-y-1/2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                disabled={idx === 0}
                                                className="bg-white border border-slate-200 rounded-full p-0.5 shadow-sm text-slate-400 hover:text-blue-600 disabled:opacity-0"
                                            >
                    ...
                    <div className="flex-1 bg-slate-100 overflow-y-auto p-8 custom-scrollbar">
                        <div className="max-w-3xl mx-auto">
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 min-h-[600px] flex flex-col overflow-hidden">
                                <div className="p-6 border-b border-slate-100 bg-gradient-to-r from-blue-500 to-indigo-600 text-white group relative">
                                    {editingId === 'title' ? (
