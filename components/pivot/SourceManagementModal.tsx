...
export const SourceManagementModal: React.FC<SourceManagementModalProps> = ({
  ...
  const localPrimaryDataset = primarySource
    ? datasets.find((d) => d.id === primarySource.datasetId)
    : primaryDataset;
  ...
  const previewStats = useMemo(() => {
    ...
    primaryRows.forEach((r: any) => {
      const val = String(r[newSource.key1] || '')
        .toLowerCase()
        .trim();
          ...
          {localSources.length > 0 && (
            <div className="space-y-3">
              ...
              {localSources.map((src) => {
                ...
                const latestBatch =
                  srcBatches.length > 0 ? srcBatches[srcBatches.length - 1] : null;
                      ...
                      <div className="flex-1">
                        ...
                        {latestBatch && (
                          <div className="text-xs text-slate-500 mb-2">
                            Import: {latestBatch.date.split('T')[0]} • {latestBatch.rows.length}{' '}
                            lignes
                          </div>
                        )}
                          ...
                          <div className="text-xs text-slate-600 space-y-1">
                            <div>
                              <span className="font-semibold">
                                {src.joinConfig.joinType?.toUpperCase() || 'LEFT'} JOIN
                              </span>
                              {' • '}
            ...
            <div className="p-4 bg-slate-50 rounded-lg border border-slate-200 space-y-3">
              ...
              <div>
                ...
                >
                  ...
                  {datasets
                    .filter((d) => !localSources.some((s) => s.datasetId === d.id))
                    .map((d) => (
                ...
                <>
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setNewSource({ ...newSource, joinType: e.target.value as 'left' | 'inner' })
                      }
                    >
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      >
                        <option value="">-- Choisir --</option>
                        {datasets
                          .find((d) => d.id === newSource.targetId)
                          ?.fields.map((f: string) => (
                            <option key={f} value={f}>
                              {f}
                            </option>
                  ...
                  {previewStats && (
                    <div className="p-3 bg-white rounded-lg border border-slate-200 animate-in fade-in slide-in-from-top-1">
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <span
                          className={`text-xs font-bold ${getQualityColor(previewStats.quality)} flex items-center gap-1`}
                        >
                      ...
                      <div className="grid grid-cols-2 gap-4 text-sm">
                        <div className="flex flex-col">
                          <span className="text-slate-500 text-[0.85em] uppercase font-bold">
                            Correspondance
                          </span>
                          <span className="font-bold text-slate-700">
                            {previewStats.matchCount} / {previewStats.totalPrimary} (
                            {previewStats.matchRate.toFixed(1)}%)
                          </span>
                        </div>
                        <div className="flex flex-col">
                          <span className="text-slate-500 text-[0.85em] uppercase font-bold">
                          ...
                          <span className="font-bold text-slate-700">
                            ~{previewStats.estimatedRows} lignes
                          </span>
                        </div>
                      </div>
                      {previewStats.quality === 'low' && (
                        <p className="mt-2 text-[0.85em] text-red-500 italic flex items-start gap-1">
                          ...
                          Peu de correspondances trouvées. Vérifiez que les clés sélectionnées
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  ...
                  disabled={
                    !newSource.targetId ||
                    (localSources.length > 0 && (!newSource.key1 || !newSource.key2))
                  }
