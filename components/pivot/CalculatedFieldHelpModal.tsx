...
export const CalculatedFieldHelpModal: React.FC<HelpModalProps> = ({ isOpen, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'functions' | 'regex' | 'examples' | 'advanced'>(
    'functions'
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600 mt-1">
                Guide complet des fonctions et exemples d'utilisation
              </p>
            </div>
          </div>
          <button
  ...
  <div className="space-y-6">
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FunctionCard
          ...
          desc="Retourne une valeur si la condition est vraie, sinon une autre"
          example='SI([Prix] > 100, "Cher", "Abordable")'
        />
        <FunctionCard
          name="SOMME"
          syntax="SOMME(v1, v2, ...)"
          desc="Additionne toutes les valeurs fournies"
          example="SOMME([Prix], [Taxes], [Frais])"
        />
        <FunctionCard
          name="MOYENNE"
          syntax="MOYENNE(v1, v2, ...)"
          desc="Calcule la moyenne des valeurs"
          example="MOYENNE([Note1], [Note2], [Note3])"
        />
        <FunctionCard
          name="ARRONDI"
          syntax="ARRONDI(nombre, décimales)"
          desc="Arrondit un nombre au nombre de décimales spécifié"
          example="ARRONDI([Prix], 2)"
        />
        <FunctionCard
          name="MIN"
          syntax="MIN(v1, v2, ...)"
          desc="Retourne la valeur minimale"
          example="MIN([Prix1], [Prix2], [Prix3])"
        />
        <FunctionCard
          name="MAX"
          syntax="MAX(v1, v2, ...)"
          desc="Retourne la valeur maximale"
          example="MAX([Vente1], [Vente2])"
        />
        <FunctionCard
          name="ABS"
          syntax="ABS(nombre)"
          desc="Retourne la valeur absolue d'un nombre"
          example="ABS([Différence])"
        />
      </div>
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FunctionCard
          ...
          desc="Concatène plusieurs textes avec un séparateur optionnel"
          example='CONCAT([Prénom], [Nom], " ")'
        />
        <FunctionCard
          name="MAJUSCULE"
          syntax="MAJUSCULE(texte)"
          desc="Convertit le texte en majuscules"
          example="MAJUSCULE([Nom])"
        />
        <FunctionCard
          name="MINUSCULE"
          syntax="MINUSCULE(texte)"
          desc="Convertit le texte en minuscules"
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FunctionCard
          ...
          desc="Remplace toutes les occurrences (supporte regex)"
          example='REMPLACER([Code], "[0-9]+", "")'
        />
        <FunctionCard
          name="SUBSTITUER"
          syntax="SUBSTITUER(texte, ancien, nouveau)"
          desc="Remplace du texte exact (sans regex)"
          example='SUBSTITUER([Statut], "En cours", "Active")'
        />
        <FunctionCard
          name="TROUVE"
          syntax="TROUVE(cherche, texte, [début])"
          desc="Trouve la position d'une sous-chaîne (-1 si absent)"
          example='TROUVE("@", [Email])'
        />
        <FunctionCard
          name="CONTIENT"
          syntax="CONTIENT(texte, cherche)"
          desc="Vérifie si le texte contient la sous-chaîne"
          example='CONTIENT([Email], "@gmail.com")'
        />
      </div>
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <FunctionCard
          name="EXTRAIRE"
          syntax="EXTRAIRE(texte, début, [longueur])"
          desc="Extrait une portion de texte"
          example="EXTRAIRE([Code], 3, 5)"
        />
        <FunctionCard
          name="GAUCHE"
          syntax="GAUCHE(texte, nb)"
          desc="Extrait les n premiers caractères"
          example="GAUCHE([Code], 3)"
        />
        <FunctionCard
          name="DROITE"
          syntax="DROITE(texte, nb)"
          desc="Extrait les n derniers caractères"
          example="DROITE([Année], 2)"
        />
        <FunctionCard
          name="LONGUEUR"
          syntax="LONGUEUR(texte)"
          desc="Retourne le nombre de caractères"
          example="LONGUEUR([Nom])"
        />
        <FunctionCard
          name="SUPPRESPACE"
          syntax="SUPPRESPACE(texte)"
          desc="Supprime les espaces de début et fin"
...
const FunctionCard: React.FC<{ name: string; syntax: string; desc: string; example: string }> = ({
  name,
  syntax,
  desc,
  example
  ...
  <div className="border border-slate-200 rounded-lg p-4 hover:border-indigo-300 hover:shadow-md transition-all bg-slate-50">
    ...
    <div className="text-xs font-mono text-slate-600 bg-white border border-slate-200 rounded px-2 py-1 mb-2">
      {syntax}
    </div>
  ...
  <div className="space-y-6">
    <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
      ...
      <p className="text-sm text-slate-700">
        Les regex permettent de rechercher des patterns complexes dans du texte. Utilisez-les avec
    ...
    <div>
      ...
      <div className="space-y-3">
        ...
        <RegexPattern
          pattern="[a-z]+"
          desc="Une ou plusieurs lettres minuscules"
          example='REMPLACER([Texte], "[a-z]+", "X")'
          result="Hello123 → HX123"
        />
        <RegexPattern
          pattern="[A-Z]+"
          desc="Une ou plusieurs lettres majuscules"
          example='REMPLACER([Code], "[A-Z]+", "")'
          result="ABC123 → 123"
        />
        <RegexPattern
          pattern="[^a-zA-Z0-9 ]"
          desc="Tout SAUF lettres, chiffres et espaces"
          example='REMPLACER([Texte], "[^a-zA-Z0-9 ]", "")'
          result="Hello@World! → HelloWorld"
        />
        <RegexPattern
          pattern=" "
          desc="Espace"
          example='REMPLACER([Tel], " ", "")'
          result="06 12 34 → 0612 34"
        />
        <RegexPattern
          pattern="@.*"
          desc="@ suivi de n'importe quoi"
          example='REMPLACER([Email], "@.*", "@new.com")'
          result="user@old.com → user@new.com"
        />
        <RegexPattern
          pattern="[-_]"
          desc="Tiret ou underscore"
          example='REMPLACER([Code], "[-_]", " ")'
          result="hello-world_test → hello world test"
        />
        <RegexPattern
          pattern="\s+"
          desc="Un ou plusieurs espaces blancs"
          example='REMPLACER([Texte], "\s+", " ")'
          result="A  B   C → A B C"
        />
      </div>
    </div>
    ...
    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
      ...
      <p className="text-sm text-slate-700 mb-3">
        Pour remplacer plusieurs chaînes différentes, imbriquez les fonctions REMPLACER :
      </p>
      <code className="block text-xs font-mono text-slate-700 bg-white border border-slate-200 rounded p-3 whitespace-pre-wrap">
        REMPLACER(REMPLACER(REMPLACER([Statut], "En cours", "Active"), "Terminé", "Done"), "Annulé",
        "Cancelled")
      </code>
      <p className="text-xs text-slate-600 mt-2">
        Chaque REMPLACER s'exécute de l'intérieur vers l'extérieur.
      </p>
    </div>
  </div>
);

const RegexPattern: React.FC<{
  pattern: string;
  desc: string;
  example: string;
  result: string;
}> = ({ pattern, desc, example, result }) => (
  <div className="bg-white border border-slate-200 rounded-lg p-3 hover:border-purple-300 transition-colors">
    ...
    <div className="text-xs font-mono text-slate-600 bg-slate-50 border border-slate-200 rounded px-2 py-1 mb-1">
      {example}
    </div>
  ...
  <div className="border border-slate-200 rounded-lg p-4 hover:border-indigo-300 hover:shadow-lg transition-all bg-gradient-to-br from-white to-slate-50">
    ...
    <div className="space-y-2">
      <div>
        ...
        <code className="block text-xs font-mono text-slate-700 bg-slate-100 border border-slate-200 rounded p-2 whitespace-pre-wrap">
          {formula}
        </code>
      </div>
      <div>
        <span className="text-xs font-medium text-slate-500 block mb-1">Données :</span>
        <code className="block text-xs font-mono text-slate-600 bg-white border border-slate-200 rounded p-2">
          {JSON.stringify(data)}
        </code>
      </div>
      <div>
        <span className="text-xs font-medium text-slate-500 block mb-1">Résultat :</span>
        <div className="text-sm font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 rounded p-2">
          {result}
        </div>
      </div>
...
const AdvancedTab: React.FC = () => (
  <div className="space-y-6">
    <div className="bg-purple-50 border border-purple-200 rounded-lg p-4">
      ...
      <p className="text-sm text-slate-700">
        Exemples de formules complexes combinant plusieurs fonctions.
      </p>
    </div>
    ...
    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
      ...
      <ul className="text-sm text-slate-700 space-y-2">
        <li className="flex items-start gap-2">
          ...
          <span>
            <strong>Testez progressivement :</strong> Construisez votre formule étape par étape en
            vérifiant l'aperçu.
          </span>
        </li>
        <li className="flex items-start gap-2">
          <span className="text-amber-600 mt-0.5">•</span>
          <span>
            <strong>Gérez les cas limites :</strong> Utilisez SI pour vérifier les valeurs nulles ou
            vides.
          </span>
        </li>
        <li className="flex items-start gap-2">
          <span className="text-amber-600 mt-0.5">•</span>
          <span>
            <strong>Documentez vos formules :</strong> Utilisez des noms de champs explicites.
          </span>
        </li>
        <li className="flex items-start gap-2">
          <span className="text-amber-600 mt-0.5">•</span>
          <span>
            <strong>Évitez la complexité inutile :</strong> Si possible, créez plusieurs champs
            intermédiaires plutôt qu'une seule formule géante.
          </span>
        </li>
      </ul>
    </div>
...
const AdvancedExample: React.FC<{
  ...
  <div className="border-l-4 border-purple-400 pl-4 py-3 bg-white rounded-r-lg">
    ...
    <code className="block text-xs font-mono text-slate-700 bg-slate-100 border border-slate-200 rounded p-3 mb-3 whitespace-pre-wrap">
      {formula}
    </code>
