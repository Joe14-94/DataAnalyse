...
export const QuickChartModal: React.FC<QuickChartModalProps> = ({ isOpen, onClose, items }) => {
  ...
  const chartData = useMemo(() => {
    ...
    items.forEach((item) => {
      ...
      group[item.colLabel] = (Number(group[item.colLabel]) || 0) + val;
  ...
  return (
    ...
    >
      <div
        id="quick-chart-modal-container"
        className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-medium">
                {items.length} point(s) de données
              </p>
            </div>
          </div>
          <button
                  ...
                  return (
                    ...
                    >
                      ...
                      <YAxis
                        ...
                        tickFormatter={
                          isPercent ? (val) => `${(val * 100).toFixed(0)}%` : undefined
                        }
                      />
                      <Tooltip
                        cursor={{ fill: '#f8fafc' }}
              ...
              {showExportMenu && (
                <div className="absolute bottom-full left-0 mb-2 bg-white border border-slate-200 rounded-lg shadow-lg z-50 py-1 w-40 overflow-hidden">
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs hover:bg-slate-50 flex items-center gap-2 font-medium"
                  >
                  ...
                  <button
                    onClick={() => {
                      exportView('png', 'quick-chart-render-area', 'Export Graphique', companyLogo);
                      setShowExportMenu(false);
                    }}
                    className="w-full text-left px-3 py-2 text-xs hover:bg-slate-50 flex items-center gap-2 font-medium"
                  >
