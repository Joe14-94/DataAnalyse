   ...
   return (
      <div className="bg-white p-2 rounded-lg border border-slate-200 shadow-sm flex flex-wrap items-center justify-between gap-2 shrink-0">
         <div className="flex items-center gap-2">
            ...
            <div>
               <h2 className="text-lg font-bold text-slate-800 leading-tight">Tableau Croisé Dynamique</h2>
            ...
            <div className="ml-4 flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
               <button onClick={() => setIsTemporalMode(false)} className={`px-2 py-1 text-xs font-bold rounded transition-all ${!isTemporalMode ? 'bg-white text-blue-600 shadow' : 'text-slate-500'}`}>
                  <Table2 className="w-3 h-3 inline mr-1" />Standard
               </button>
               <button onClick={() => setIsTemporalMode(true)} className={`px-2 py-1 text-xs font-bold rounded transition-all ${isTemporalMode ? 'bg-white text-blue-600 shadow' : 'text-slate-500'}`}>
         ...
         <div className="flex items-center gap-2">
            <button onClick={handleToChart} disabled={!primaryDataset} className="flex items-center gap-1 px-3 py-1.5 rounded text-sm font-bold bg-blue-50 text-blue-700 hover:bg-blue-100 border border-blue-200 disabled:opacity-50">
               <PieChart className="w-3.5 h-3.5" /> Graphique
            </button>

            <div className="relative">
               <button onClick={() => setShowExportMenu(!showExportMenu)} disabled={!primaryDataset} className="px-3 py-1.5 text-sm text-slate-600 hover:text-blue-600 border border-slate-300 rounded bg-white hover:bg-slate-50 flex items-center gap-1 disabled:opacity-50">
