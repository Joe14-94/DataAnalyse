...
export const PivotHeader: React.FC<PivotHeaderProps> = ({
  ...
  openCalcModal,
  openFormattingModal,
  openSpecificDashboardModal,
  openSaveAsDatasetModal,
    ...
    <div className="bg-white p-2 rounded-lg border border-slate-200 shadow-sm flex flex-wrap items-center justify-between gap-2 shrink-0">
      <div className="flex items-center gap-2">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500">
            {isTemporalMode ? 'Comparaison temporelle' : 'Analyse multidimensionnelle'}
          </p>
        </div>
        <div className="ml-4 flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
          <button
            onClick={() => setIsTemporalMode(false)}
            className={`px-2 py-1 text-xs font-bold rounded transition-all ${!isTemporalMode ? 'bg-white text-brand-600 shadow' : 'text-slate-500'}`}
          >
        ...
        <div className="relative ml-2">
          ...
          <input
            ...
            className="pl-7 pr-3 py-1.5 bg-slate-100 border-none rounded-lg text-xs w-40 md:w-64 focus:ring-2 focus:ring-brand-500 focus:bg-white transition-all outline-none"
            value={searchTerm}
        ...
        <div className="flex items-center gap-1 border-l border-slate-200 pl-2">
          <button
            onClick={undo}
            disabled={!canUndo}
            className="p-1.5 text-slate-500 hover:text-brand-600 border border-slate-200 rounded-md bg-white disabled:opacity-30 disabled:cursor-not-allowed transition-all"
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          title={
            isEditMode
              ...
              : 'Activer le mode édition pour renommer les colonnes'
          }
        >
          <Edit3 className={`w-3 h-3 ${isEditMode ? 'animate-pulse' : ''}`} />{' '}
          {isEditMode ? 'Édition : ON' : 'Mode Édition'}
        </button>

        <button
          onClick={openCalcModal}
          disabled={!primaryDataset}
          className="flex items-center gap-1 px-3 py-1.5 rounded text-xs font-bold bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 disabled:opacity-50"
        >
        ...
        <button
          ...
          disabled={!primaryDataset}
          className="flex items-center gap-1 px-3 py-1.5 rounded text-xs font-bold bg-slate-50 text-slate-700 hover:bg-slate-100 border border-slate-300 disabled:opacity-50"
        >
          <Palette className="w-3 h-3 text-indigo-500" /> Mise en forme
        </button>

        <button
          onClick={openSaveAsDatasetModal}
          disabled={!primaryDataset}
          className="flex items-center gap-1 px-3 py-1.5 rounded text-xs font-bold bg-orange-50 text-orange-700 hover:bg-orange-100 border border-orange-200 disabled:opacity-50"
        >
          <Database className="w-3 h-3" /> Sauver comme Dataset
        </button>

        <button
          onClick={openSpecificDashboardModal}
          disabled={!primaryDataset}
          className="relative flex items-center gap-1 px-3 py-1.5 rounded text-xs font-bold bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200 disabled:opacity-50"
        >
          <MonitorPlay className="w-3 h-3" /> Créer Dashboard
          {selectedItemsCount > 0 && (
            <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs w-4 h-4 flex items-center justify-center rounded-full border border-white shadow-sm">
              {selectedItemsCount}
            </span>
          )}
        </button>
        ...
        <div className="relative">
          <button
            ...
            disabled={!primaryDataset}
            className="px-3 py-1.5 text-xs text-slate-600 hover:text-brand-600 border border-slate-300 rounded bg-white hover:bg-slate-50 flex items-center gap-1 disabled:opacity-50"
          >
        ...
        <div className="relative">
          <button
            ...
            className="p-1.5 text-slate-500 hover:text-green-600 border border-slate-300 rounded bg-white"
            title="Charger"
          >
          ...
          {showLoadMenu && (
            <div className="absolute right-0 mt-1 w-64 bg-white border border-slate-200 rounded-lg shadow-lg z-50 py-1 max-h-80 overflow-y-auto">
              <div className="px-3 py-1.5 text-xs font-bold text-slate-400 uppercase bg-slate-50 border-b border-slate-100">
                  ...
                  .map((a) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left px-3 py-2 text-xs hover:bg-slate-50 flex items-center justify-between gap-2 border-b border-slate-100 last:border-0"
                      >
        ...
        {!isSaving ? (
          <button
            ...
            disabled={!primaryDataset}
            className="p-1.5 text-slate-500 hover:text-brand-600 border border-slate-300 rounded bg-white disabled:opacity-50"
            title="Sauvegarder"
          >
          ...
          <div className="flex items-center gap-1">
            <input
              type="text"
              className="p-1 text-xs border border-brand-300 rounded w-24"
              placeholder="Nom..."
              value={analysisName}
              onChange={(e) => setAnalysisName(e.target.value)}
              onKeyDown={(e) => {
                if (e.key === 'Enter') handleSaveAnalysis();
              }}
              autoFocus
            />
            <button onClick={handleSaveAnalysis} className="p-1 bg-brand-600 text-white rounded">
