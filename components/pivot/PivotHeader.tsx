      ...
      <div className="bg-white p-2 rounded-lg border border-slate-200 shadow-sm flex flex-wrap items-center justify-between gap-2 shrink-0">
         <div className="flex items-center gap-2">
            ...
            <div>
               ...
               <p className="text-xs text-slate-500">{isTemporalMode ? 'Comparaison temporelle' : 'Analyse multidimensionnelle'}</p>
            </div>
            <div className="ml-4 flex items-center gap-1 bg-slate-100 rounded-lg p-0.5">
               <button onClick={() => setIsTemporalMode(false)} className={`px-2 py-1 text-xs font-bold rounded transition-all ${!isTemporalMode ? 'bg-white text-brand-600 shadow' : 'text-slate-500'}`}>
         ...
         <div className="flex items-center gap-2">
            ...
            <div className="relative">
               ...
               {showLoadMenu && (
                  <div className="absolute right-0 mt-1 w-64 bg-white border border-slate-200 rounded-lg shadow-lg z-50 py-1 max-h-80 overflow-y-auto">
                     <div className="px-3 py-1.5 text-xs font-bold text-slate-400 uppercase bg-slate-50 border-b border-slate-100">Analyses sauvegardées</div>
                           ...
                           return (
                              <button key={a.id} onClick={() => handleLoadAnalysis(a.id)} className="w-full text-left px-3 py-2 text-xs hover:bg-slate-50 flex items-center justify-between gap-2 border-b border-slate-100 last:border-0">
                                 <div className="flex flex-col min-w-0">
                                    ...
                                    <span className="text-xs text-brand-600 font-medium truncate">Dataset: {ds?.name || 'Inconnu'}</span>
                                 </div>
            ...
            {!isSaving ? (
               <button onClick={() => setIsSaving(true)} disabled={!primaryDataset} className="p-1.5 text-slate-500 hover:text-brand-600 border border-slate-300 rounded bg-white disabled:opacity-50" title="Sauvegarder"><Save className="w-4 h-4" /></button>
            ) : (
               <div className="flex items-center gap-1">
                  <input
                     ...
                     autoFocus
                  />
                  <button onClick={handleSaveAnalysis} className="p-1 bg-brand-600 text-white rounded"><Check className="w-3 h-3" /></button>
