...
import { Budget as BudgetType, BudgetVersion } from '../../types';
...
interface BudgetListProps {
  ...
  onCreateBudget: (name: string, fiscalYear: number, chartId: string, calendarId?: string) => void;
}
...
export const BudgetList: React.FC<BudgetListProps> = ({
  ...
  const handleCreateClick = () => {
    ...
    if (budgetName) {
      onCreateBudget(budgetName, year, defaultChart.id, fiscalCalendars[0]?.id);
    }
  };
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-2xl font-bold text-slate-800 mt-1">
                {
                  budgets.filter((b) =>
                    b.versions.some((v) => v.status === 'draft' || v.status === 'submitted')
                  ).length
                }
              </div>
            </div>
      ...
      <Card title="Liste des budgets" icon={<FileText className="w-5 h-5 text-brand-600" />}>
        {budgets.length === 0 ? (
          <div className="text-center py-12">
            ...
            <Button className="bg-brand-600 hover:bg-brand-700" onClick={handleCreateClick}>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        {activeVersion && (
                          <span
                            className={`text-xs font-bold px-2 py-1 rounded ${statusColors[activeVersion.status as keyof typeof statusColors]}`}
                          >
