...
## 2026-02-25 - [Optimize ETL Pivot & Unpivot]
**Learning:** For large-scale data transformations (e.g., Unpivot with 50k rows x 12 columns), the primary bottleneck is often object allocation and property assignment (creating 600,000 objects) rather than loop syntax. In Pivot operations, moving from a multi-pass array-based aggregation to a single-pass accumulator system reduces memory pressure and CPU time by ~17%.
**Action:** Always prioritize single-pass accumulators for any aggregation logic. When creating millions of objects, consider if the schema can be simplified or if objects can be partially pre-allocated/cloned.
