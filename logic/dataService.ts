...
export const getDemoData = () => {
  ...
  const widgets: DashboardWidget[] = [
    ...
    },
    {
      ...
      type: 'kpi',
      size: 'sm',
      config: { source: { datasetId: id2, mode: 'latest' }, metric: 'count', showTrend: false }
    },
    {
      id: 'w3',
      ...
      size: 'full',
      config: {
        source: { datasetId: id1, mode: 'latest' },
        metric: 'count',
        dimension: 'DateModif',
        chartType: 'line'
      }
...
export const createBackupJson = (state: AppState) => {
  ...
  const isLarge =
    compactState.batches.length > 5 || compactState.batches.some((b) => b.d && b.d.length > 500);
