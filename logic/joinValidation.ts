...
export const validateJoin = (
  ...
  primaryKeys.forEach((key) => {
    if (secondaryKeys.has(key)) {
      ...
      if (unmatchedPrimary.length < 5) {
        // Garder max 5 exemples
  ...
  if (matchCount === 0) {
    errors.push(
      `Aucune correspondance trouvée entre les clés - Vérifiez que les valeurs correspondent`
    );
  } else if (matchRate < 0.1) {
    errors.push(
      `Taux de correspondance très faible (${(matchRate * 100).toFixed(1)}%) - Les clés semblent incompatibles`
    );
  } else if (matchRate < 0.5) {
    warnings.push(
      `Taux de correspondance modéré (${(matchRate * 100).toFixed(1)}%) - ${primaryKeys.size - matchCount} clés principales sans correspondance`
    );
  }

  // 5. Détecter les doublons
  const duplicatesInPrimary = Array.from(primaryKeyCount.values()).filter(
    (count) => count > 1
  ).length;
  const duplicatesInSecondary = Array.from(secondaryKeyCount.values()).filter(
    (count) => count > 1
  ).length;

  if (duplicatesInPrimary > 0) {
    warnings.push(
      `${duplicatesInPrimary} clés en double dans la source principale - Risque de duplication de lignes (relation 1:N)`
    );
  }

  if (duplicatesInSecondary > 0) {
    warnings.push(
      `${duplicatesInSecondary} clés en double dans la source liée - Seule la première correspondance sera utilisée`
    );
  }

  // 6. Vérifier les types (basique)
  const samplePrimary = primaryRows.slice(0, 10).map((r) => String(r[join.joinKeyPrimary] || ''));
  const sampleSecondary = secondaryRows
    .slice(0, 10)
    .map((r) => String(r[join.joinKeySecondary] || ''));
  ...
  if (typePrimary !== typeSecondary && typePrimary !== 'text' && typeSecondary !== 'text') {
    warnings.push(
      `Types de clés différents (${typePrimary} vs ${typeSecondary}) - Conversion automatique appliquée`
    );
  }
