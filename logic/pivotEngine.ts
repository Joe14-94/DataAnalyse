...
export const calculatePivotData = (config: PivotCalculationProps): PivotResult | null => {
  ...
  const activeMetrics: PivotMetric[] =
    config.metrics && config.metrics.length > 0
      ? config.metrics
      : config.valField
        ? [{ field: config.valField, aggType: config.aggType }]
        : [];
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    for (let j = 0; j < numMetrics; j++) {
      ...
      metricVals[j] =
        mc.aggType === 'count' || mc.aggType === 'list'
          ? 0
          : parseSmartNumber(rawVals[j], mc.valUnit);
    }
  ...
  const initStats = () => ({
    ...
    rowTotalMetrics: metricConfigs.map((mc) =>
      mc.aggType === 'min'
        ? Infinity
        : mc.aggType === 'max'
          ? -Infinity
  ...
  const finalizeStats = (stats: any, headers: string[]): GroupStats => {
    ...
    metricConfigs.forEach((mc, mIdx) => {
      ...
      else if (mc.aggType === 'list' && stats.rowTotalMetrics[mIdx] instanceof Set)
        rawTotalVal = (stats.rowTotalMetrics[mIdx] as Set<string>).size;
    ...
    if (showVariations) {
      if (colFields && colFields.length > 0) {
        ...
        for (let i = 1; i < metricConfigs.length; i++) {
          const mLabel =
            metricConfigs[i].label || `${metricConfigs[i].field} (${metricConfigs[i].aggType})`;
          const prevMLabel =
            metricConfigs[i - 1].label ||
            `${metricConfigs[i - 1].field} (${metricConfigs[i - 1].aggType})`;
    ...
    return {
      ...
      rowTotal:
        metricConfigs.length === 1 ? Object.values(finalTotalMetrics)[0] : finalTotalMetrics,
      rawRowTotal,
      rawMetrics
    };
  };
...
export const formatPivotOutput = (
  ...
  if (aggType !== 'count' && valField) {
    if (
      overrideConfig &&
      (overrideConfig.decimalPlaces !== undefined ||
        overrideConfig.displayScale !== undefined ||
        overrideConfig.unit)
    ) {
