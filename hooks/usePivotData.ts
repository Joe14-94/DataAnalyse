...
export const usePivotData = ({
  ...
  const [temporalColTotals, setTemporalColTotals] = useState<{
    [sourceId: string]: { [metricLabel: string]: number };
  }>({});
  const [isCalculating, setIsCalculating] = useState(false);

  const primarySourceConfig = sources.find((s) => s.isPrimary);
  const primaryDataset =
    (primarySourceConfig ? datasets.find((d) => d.id === primarySourceConfig.datasetId) : null) ||
    null;
  ...
  useEffect(() => {
    ...
    const activeMetrics =
      metrics.length > 0 ? metrics : valField ? [{ field: valField, aggType }] : [];

    if (
      rowFields.length === 0 ||
      activeMetrics.length === 0 ||
      (temporalConfig?.sources?.length || 0) < 2
    ) {
      setTemporalResults([]);
      return;
    }
    ...
    const timer = setTimeout(() => {
      ...
      const { results, colTotals } = calculateTemporalComparison(
        sourceDataMap,
        activeConfig,
        dateColumn,
        showSubtotals,
