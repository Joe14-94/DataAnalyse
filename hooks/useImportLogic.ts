...
export type ImportStep = 'input' | 'mapping' | 'confirm';
export type FileEncoding = 'auto' | 'UTF-8' | 'windows-1252';
export type DeleteConfirm = { type: 'row' | 'header'; index?: number } | null;
...
export function useImportLogic() {
    ...
    const handleAnalyzeText = useCallback(() => {
        if (!text.trim()) return;
        processImportData(parseRawData(text));
    ...
    const handleCleanColumn = useCallback((action: 'trim' | 'upper' | 'lower' | 'proper' | 'empty_zero') => {
        ...
        const newRows = rawData.rows.map(row => {
            ...
            if (action === 'empty_zero') val = (!val || val.trim() === '') ? '0' : val;
    ...
    const handleRemoveHeader = useCallback(() => {
        ...
        setRawData({ headers: rawData.rows[0], rows: rawData.rows.slice(1), totalRows: rawData.rows.length - 1 });
        setDeleteConfirm(null);
    }, [rawData]);

    // ── Mapping ───────────────────────────────────────────────────────────────

    // Auto-compute mapping when reaching step 'mapping'
    useEffect(() => {
        ...
        rawData.headers.forEach((header, index) => {
            ...
            if (targetDatasetId === 'NEW') {
                ...
                if (savedMappings[h] && availableFields.includes(savedMappings[h])) {
                    ...
                    if (h.length > 1) { initialMapping[index] = h; mappedName = h; }
                    else              { initialMapping[index] = 'ignore'; }
                }
            }
            ...
            if (mappedName && mappedName !== 'ignore') {
                if (existingConfigs[mappedName]) {
                    ...
                    if (detectedType === 'number') {
                        initialConfigs[mappedName] = { type: 'number', unit: detectUnit(allColumnValues) };
                    } else {
    ...
    const handleConfigChange = useCallback((fieldName: string, key: keyof FieldConfig, value: string) => {
        setTempFieldConfigs(prev => ({ ...prev, [fieldName]: { ...prev[fieldName], [key]: value } }));
    }, []);
    ...
    const handleFinalizeImport = useCallback(() => {
        ...
        if (targetDatasetId === 'NEW' || (targetDatasetId !== 'NEW' && updateMode === 'overwrite' && finalDatasetId !== targetDatasetId)) {
            ...
            fieldsToProcess.forEach(({ name }) => addFieldToDataset(finalDatasetId, name, tempFieldConfigs[name]));
        ...
        rawData.headers.forEach((header, idx) => {
            ...
            if (mappedField && mappedField !== 'ignore') newLearnedMappings[header.trim()] = mappedField;
        });
        if (Object.keys(newLearnedMappings).length > 0) updateSavedMappings(newLearnedMappings);
    ...
    }, [rawData, targetDatasetId, updateMode, datasets, mapping, newDatasetName, tempFieldConfigs,
        deleteDataset, createDataset, addFieldToDataset, updateDatasetConfigs, mapDataToSchema,
        updateSavedMappings, addBatch, date, switchDataset]);
