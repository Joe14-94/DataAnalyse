...
import {
  applyPivotFilters,
  getEffectiveBatches,
  getChartColorsForWidget
} from '../logic/widgetEngine';
...
export const useWidgetData = (
  ...
  return useMemo(() => {
    ...
    if (widget.type === 'chart' && widget.config.reportItems && !widget.config.source) {
      ...
      const isStackedSingleBar = [
        'stacked-bar',
        'stacked-column',
        'percent-bar',
        'percent-column'
      ].includes(chartType as string);

      let chartData;
      let colorCount = items.length;

      if (isStackedSingleBar) {
        const singleGroup: Record<string, string | number> = { name: 'Sélection', value: 0 };
        items.forEach((item) => {
          const val =
          ...
          if (typeof singleGroup.value === 'number') {
    ...
    if (widget.config.pivotChart) {
      ...
      if (
        pivotChart.updateMode === 'fixed' &&
        widget.config.source?.mode === 'specific' &&
        widget.config.source?.batchId
      ) {
      ...
      let pivotResult: PivotResult | null = null;
      ...
      if (pivotChart.isTemporalMode && pivotChart.temporalComparison) {
        ...
        let effectiveSources = (tc.sources || []) as TemporalComparisonSource[];
        if (pivotChart.updateMode === 'latest') {
          const sortedBatches = [...(batches || [])]
            .filter((b) => b.datasetId === widget.config.source?.datasetId)
            .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

          effectiveSources = (tc.sources || []).map(
            (s: TemporalComparisonSource, idx: number) => ({
              ...s,
        ...
        const colHeaders = effectiveSources.map((s, idx) => {
          if (pivotChart.updateMode === 'latest') {
            const batch = (batches || []).find((b) => b.id === s.batchId);
            if (batch) {
              const dateStr = new Date(batch.date).toLocaleDateString('fr-FR', {
                day: '2-digit',
                month: '2-digit'
              });
        ...
        const displayRows: PivotRow[] = (results || []).map((r: TemporalComparisonResult) => {
          ...
          (effectiveSources || []).forEach((s) => {
            ...
            metrics[label] =
              r.values && typeof r.values?.[s.id]?.[pc.valField] === 'number'
                ? r.values[s.id][pc.valField]
                : 0;
          });
          ...
          return {
            ...
            metrics,
            rowTotal: Object.values(r.values || {}).reduce(
              (a: number, b: any) => a + (Object.values(b)[0] as number || 0),
              0
            )
          };
        });

        pivotResult = { colHeaders, displayRows, colTotals: {}, grandTotal: 0 };
      } else {
      ...
      if (pivotChart.chartType === 'sunburst') {
        ...
        if (pivotResult.displayRows && Array.isArray(pivotResult.displayRows)) {
          pivotResult.displayRows.forEach((row: PivotRow) => {
        ...
        const sunburstColors = (() => {
          if (pivotChart.colorMode === 'single')
            return Array(colorCount).fill(pivotChart.singleColor || '#3b82f6');
          if (pivotChart.colorMode === 'gradient')
            return generateGradient(
              pivotChart.gradientStart || '#3b82f6',
              pivotChart.gradientEnd || '#ef4444',
              colorCount
            );
      ...
      const hierarchicalBaseColors = (() => {
        if (pivotChart.colorMode === 'single')
          return Array(9).fill(pivotChart.singleColor || '#3b82f6');
        if (pivotChart.colorMode === 'gradient')
          return generateGradient(
            pivotChart.gradientStart || '#3b82f6',
            pivotChart.gradientEnd || '#ef4444',
      ...
      if (pivotChart.chartType === 'treemap') {
        const treeData = transformPivotToHierarchicalTreemap(
          pivotResult,
          fullPivotConfig,
          hierarchicalBaseColors,
          {
            limit: pivotChart.limit,
            showOthers: (pivotChart.limit || 0) > 0
          }
        );
        const flatData = transformPivotToTreemapData(
          pivotResult,
          fullPivotConfig,
          pivotChart.hierarchyLevel
        );
        return {
          data: flatData,
      ...
      const colorCount =
        pivotChart.chartType === 'pie' || pivotChart.chartType === 'donut'
          ? pointCount
          : seriesCount > 1
            ? seriesCount
            : pointCount;
    ...
    if (secondarySource && secondarySource.datasetId) {
      ...
      const secBatches = getEffectiveBatches(
        batches || [],
        secondarySource.datasetId,
        globalDateRange
      );
    ...
    const parseVal = (row: DataRow, field: string) => {
    ...
    const getUnit = (field: string | undefined) => {
      if (!field) return '';
      return (
        dataset.fieldConfigs?.[field]?.unit || secondaryDataset?.fieldConfigs?.[field]?.unit || ''
      );
    };

    const currentUnit =
      (metric === 'sum' || metric === 'avg') && valueField ? getUnit(valueField) : '';
    ...
    if (widget.type === 'list') {
      ...
      return {
        current: sorted,
        max: sorted.length > 0 ? sorted[0].value : 0,
        unit: currentUnit,
    ...
    if (dimension) {
      ...
      if (widget.config.chartType === 'radial') {
        const data = Object.entries(counts)
          .map(([name, value], idx) => ({
            name,
            value,
            fill: standardColors[idx % standardColors.length]
          }))
          .sort((a, b) => b.value - a.value)
          .slice(0, 5);
      ...
      let currentVal = 0;
      let prevVal = 0;
      const calc = (rows: DataRow[]) => {
        if (!rows) return 0;
        if (metric === 'count') return rows.length;
        if (metric === 'sum' && valueField)
          return rows.reduce((acc: number, r) => acc + parseVal(r, valueField), 0);
        if (metric === 'avg' && valueField)
          return (
            rows.reduce((acc: number, r) => acc + parseVal(r, valueField), 0) /
            (rows.length || 1)
          );
        if (metric === 'distinct' && valueField)
          return new Set(rows.map((r) => r[valueField])).size;
        return 0;
      };
