...
import { 
  ...
  FilterX, Hash, Percent, MousePointerClick, Variable, Sigma, Play, AlertCircle, CheckCircle2, Columns, AlertTriangle, Link as LinkIcon
} from 'lucide-react';
...
export const DataExplorer: React.FC = () => {
  ...
  // DATA BLENDING STATE (FROM TCD DRILLDOWN)
  ...
  const clearFilters = () => {
    ...
    setSortConfig(null);
    setCurrentPage(1);
    // Reset blending on clear filters (optional choice, but cleaner)
    // setBlendingConfig(null); 
  };
  ...
  // 1. Flatten Data AND Calculate Fields AND Perform Blending if needed
  ...
  // Determine Display Fields (Primary + Secondary if Blending)
            ...
            <table className="min-w-full divide-y divide-slate-200 border-collapse text-left">
               <thead className="bg-slate-50 sticky top-0 z-20 shadow-sm">
                  ...
                  <tr>
                     ...
                     {/* Colonnes Standard + Colonnes Blendées */}
                     {displayFields.map(field => {
                        const isSelected = selectedCol === field;
                        const isBlended = field.startsWith('['); // Détection basique pour styling
                        ...
                        return (
                           <th 
                              ...
                              className={`px-6 py-3 text-left text-xs font-bold tracking-wider whitespace-nowrap border-b cursor-pointer transition-colors select-none group
                                 ${isCalcDrawerOpen ? 'hover:bg-indigo-100 hover:text-indigo-800' : (isSelected ? 'bg-teal-50 text-teal-900 border-teal-300' : (isBlended ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100'))}
                              `}
                     ...
                     paginatedRows.map((row, idx) => (
                        ...
                        >
                           ...
                           {displayFields.map(field => {
                              ...
                              return (
                                 <td key={field} className={`px-6 py-3 whitespace-nowrap text-sm text-slate-700 max-w-xs truncate ${cellStyle} ${config?.type === 'number' ? 'text-right font-mono' : ''} ${isBlended ? 'text-purple-700 bg-purple-50/20' : ''}`} title={String(val)}>
                                    {displayVal}
                                 </td>
                              );
                           })}
