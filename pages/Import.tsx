...
export const Import: React.FC = () => {
  ...
  const handleRemoveDuplicates = () => {
    ...
    if (newRows.length < rawData.rows.length) {
      ...
      if (
        window.confirm(
          `${removed} doublons détectés sur cette colonne. Voulez-vous les supprimer ?`
        )
      ) {
  ...
  const handleFinalizeImport = () => {
    ...
    if (
      ...
      (targetDatasetId !== 'NEW' &&
        ...
        finalDatasetId !== targetDatasetId)
    ) {
      if (targetDatasetId === 'NEW') {
        finalDatasetId = createDataset(
          newDatasetName || 'Nouveau Tableau',
          uniqueFieldNames,
          tempFieldConfigs
        );
      } else {
  ...
  const renderMappingStep = () => {
    ...
    return (
      <div className="space-y-6 animate-in slide-in-from-right duration-300 relative">
          ...
          <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
              ...
              <div className="flex justify-end gap-3">
                ...
                <Button
                  variant="danger"
                  onClick={deleteConfirm.type === 'header' ? handleRemoveHeader : handleRemoveRow}
                >
        ...
        <Card className="p-6 border-brand-200 bg-brand-50">
          <div className="flex justify-between items-start">
            ...
            <div className="flex items-center gap-2 bg-white px-3 py-2 rounded-md shadow-sm border border-brand-100">
              <label htmlFor="step2-date" className="text-xs font-bold text-brand-700">
                Date d'extraction :
              </label>
              <input
                type="date"
          ...
          <div className="space-y-4">
            {detectedDatasetId && (
              <div className="flex items-center gap-2 text-sm text-green-700 bg-green-100 p-2 rounded border border-green-200">
                ...
                Typologie reconnue :{' '}
                <strong>{datasets.find((d) => d.id === detectedDatasetId)?.name}</strong>
              </div>
            )}

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <label
                className={`relative flex items-start p-4 cursor-pointer rounded-lg border-2 transition-all ${targetDatasetId === 'NEW' ? 'border-brand-500 bg-white shadow-md' : 'border-slate-200 bg-slate-50 hover:bg-white'}`}
              >
                <input
                  type="radio"
                  name="targetDS"
              ...
              {datasets.length > 0 && (
                ...
                >
                  ...
                  <div className="ml-3 w-full">
                    <span className="block text-sm font-medium text-slate-900">
                      Ajouter à une typologie existante
                    </span>
                    <select
        ...
        <div
          className={`transition-all duration-300 overflow-hidden ${selectedColIndex !== null ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}
        >
          <div className="bg-white border border-purple-200 rounded-lg p-4 shadow-sm bg-gradient-to-r from-white to-purple-50">
            <div className="flex items-center gap-2 mb-2 text-purple-800 text-xs font-bold uppercase tracking-wider">
              ...
              Outils de nettoyage :{' '}
              {selectedColIndex !== null ? rawData.headers[selectedColIndex] : ''}
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => handleCleanColumn('trim')}
              ...
              <button
                onClick={() => handleCleanColumn('upper')}
                className="flex items-center gap-1 px-3 py-1.5 bg-white border border-slate-200 hover:border-purple-300 hover:bg-purple-50 rounded text-xs text-slate-700 transition-colors shadow-sm"
              >
                <CaseUpper className="w-3 h-3" /> MAJUSCULE
              </button>
              <button
                onClick={() => handleCleanColumn('lower')}
                className="flex items-center gap-1 px-3 py-1.5 bg-white border border-slate-200 hover:border-purple-300 hover:bg-purple-50 rounded text-xs text-slate-700 transition-colors shadow-sm"
              >
                <CaseLower className="w-3 h-3" /> minuscule
              </button>
              <button
                onClick={() => handleCleanColumn('proper')}
                className="flex items-center gap-1 px-3 py-1.5 bg-white border border-slate-200 hover:border-purple-300 hover:bg-purple-50 rounded text-xs text-slate-700 transition-colors shadow-sm"
              >
              ...
              <button
                onClick={() => handleCleanColumn('empty_zero')}
                className="flex items-center gap-1 px-3 py-1.5 bg-white border border-slate-200 hover:border-purple-300 hover:bg-purple-50 rounded text-xs text-slate-700 transition-colors shadow-sm"
              >
                Vide → 0
              </button>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h4 className="text-base font-bold text-amber-800">
                  Évolution de la structure détectée
                </h4>
                <p className="text-sm text-amber-700 mt-1">
                  Le fichier importé comporte des différences avec "
                  ...
                  <div className="flex flex-col sm:flex-row gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="text-amber-600 bg-white focus:ring-amber-500"
                      />
          ...
          <div className="px-4 py-2 border-b border-slate-200 bg-slate-50 flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="font-mono text-slate-600 min-w-[100px] text-center">
                {(previewPage - 1) * rowsPerPage + 1} -{' '}
                {Math.min(previewPage * rowsPerPage, rawData.totalRows)} / {rawData.totalRows}
              </span>
              <button
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between">
                          <div
                            className="text-sm font-bold text-slate-900 mb-3 truncate"
                            title={header}
                          >
                            {header}
                          </div>
                          {isSelected && (
                            <Wand2 className="w-4 h-4 text-purple-600 mb-3 animate-pulse" />
                          )}
                        </div>
                          ...
                          {targetDatasetId === 'NEW' ? (
                            ...
                            >
                              ...
                              <optgroup label="Nouveau champ">
                                <option value={header} className="text-brand-600 font-bold">
                                  + Ajouter "{header}"
                                </option>
                              </optgroup>
                            </select>
                        ...
                        {isMapped && (
                          <div
                            className="bg-white border border-slate-200 rounded p-2 space-y-2 shadow-sm"
                          ...
                          >
                            <div className="flex gap-1">
                              <select
                                ...
                                onChange={(e) =>
                                  handleConfigChange(mappedVal, 'type', e.target.value)
                                }
                              >
                              ...
                              <div className="space-y-1 animate-in fade-in duration-200">
                                <div className="flex gap-1">
                                  <input
                                    ...
                                    onChange={(e) =>
                                      handleConfigChange(mappedVal, 'unit', e.target.value)
                                    }
                                  />
                                  <input
                                    type="number"
                                    ...
                                    placeholder="Décim."
                                    value={
                                      tempFieldConfigs[mappedVal]?.decimalPlaces !== undefined
                                        ? tempFieldConfigs[mappedVal]?.decimalPlaces
                                        : ''
                                    }
                                    onChange={(e) =>
                                      handleConfigChange(mappedVal, 'decimalPlaces', e.target.value)
                                    }
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    handleConfigChange(mappedVal, 'displayScale', e.target.value)
                                  }
                ...
                {paginatedPreviewRows.map((row, rowIdx) => (
                  <tr key={rowIdx} className="group hover:bg-red-50/50">
                    ...
                    {row.map((cell, cellIdx) => (
                      <td
                        key={cellIdx}
                        className={`px-4 py-2 text-sm ${mapping[cellIdx] !== 'ignore' ? 'text-slate-900 bg-slate-50/50' : 'text-slate-400'}`}
                      >
                        {cell}
                      </td>
                    ))}
