...
const FilterConfig: React.FC<{
  ...
  const addCondition = () => {
    onUpdate({
      ...
      conditions: [
        ...config.conditions,
        { field: availableColumns[0] || '', operator: 'equals' as FilterOperator, value: '' }
      ]
    });
  };
...
const SelectConfig: React.FC<any> = () => (
  <div className="text-sm text-slate-600">
    Configuration Sélection - Sélectionnez les colonnes à conserver
  </div>
);
const AggregateConfig: React.FC<any> = () => (
  <div className="text-sm text-slate-600">
    Configuration Agrégation - Groupez et agrégez vos données
  </div>
);
const SortConfig: React.FC<any> = () => (
  <div className="text-sm text-slate-600">Configuration Tri - Triez vos données</div>
);
const RenameConfig: React.FC<any> = () => (
  <div className="text-sm text-slate-600">Configuration Renommage - Renommez vos colonnes</div>
);
const SplitConfig: React.FC<any> = () => (
  <div className="text-sm text-slate-600">
    Configuration Division - Divisez une colonne en plusieurs
  </div>
);
const MergeConfig: React.FC<any> = () => (
  <div className="text-sm text-slate-600">Configuration Fusion - Fusionnez plusieurs colonnes</div>
);
...
const StepConfiguration: React.FC<{
  ...
  switch (step.type) {
    ...
    case 'sort':
      return (
        <SortConfig config={step.config} availableColumns={availableColumns} onUpdate={onUpdate} />
      );
    case 'rename':
      return (
        <RenameConfig
    ...
    case 'split':
      return (
        <SplitConfig config={step.config} availableColumns={availableColumns} onUpdate={onUpdate} />
      );
    case 'merge':
      return (
        <MergeConfig config={step.config} availableColumns={availableColumns} onUpdate={onUpdate} />
      );
    case 'distinct':
...
export const ETLPipeline: React.FC = () => {
  ...
  const handleSavePipeline = () => {
    const nodes = steps.map((step) => ({
      ...
      config:
        step.type === 'source'
          ? { type: 'source', datasetId: sourceDatasetId }
          : ({ type: step.type, config: step.config } as any),
      isValid: true
    }));
  ...
  const toggleStepExpanded = (stepId: string) => {
    setSteps(
      steps.map((step) => (step.id === stepId ? { ...step, isExpanded: !step.isExpanded } : step))
    );
  };
  ...
  // Get final result
  const finalResult =
    steps.length > 0 ? pipelineResults[steps[steps.length - 1].id] : { data: sourceData };
                  ...
                  pipelineModule.pipelines.map((p) => (
                    ...
                    >
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          if (window.confirm('Supprimer ce pipeline ?')) deletePipeline(p.id);
                        }}
            ...
            >
              ...
              {datasets.map((dataset) => (
                <option key={dataset.id} value={dataset.id}>
                  {dataset.name} (
                  {batches
                    .filter((b) => b.datasetId === dataset.id)
                ...
                {steps.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {steps.map((step, index) => {
                      ...
                      const prevData =
                        index === 0 ? sourceData : pipelineResults[steps[index - 1].id]?.data || [];
                            ...
                            <div className="flex items-center gap-3 flex-1">
                              ...
                              {result && (
                                <span className="text-xs text-slate-600">
                                  {result.data.length} lignes • {getColumns(result.data).length}{' '}
                                  colonnes
                                </span>
                              )}
                              {result?.error && (
                              ...
                              {result && !result.error && (
                                <div className="mt-4">
                                  <h5 className="font-bold text-sm text-slate-700 mb-2">
                                    Aperçu ({Math.min(result.data.length, previewLimit)} /{' '}
                                    {result.data.length} lignes)
                                  </h5>
