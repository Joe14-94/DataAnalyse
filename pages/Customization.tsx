...
export const Customization: React.FC = () => {
  ...
  const handleRemoveLogo = () => {
    if (
      window.confirm("Voulez-vous vraiment supprimer le logo et revenir à l'affichage par défaut ?")
    ) {
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Gérez l'identité visuelle de votre application.
            </p>
          </div>
        </div>
        ...
        >
          <div className="space-y-8">
            <p className="text-sm text-txt-secondary">
              Personnalisez l'affichage global de l'application. Ces réglages s'appliquent à tous
              les composants pour garantir une cohérence visuelle.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-3">
                <label className="text-sm font-bold text-txt-main block uppercase tracking-wider text-xs">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    >
                      <span
                        className={`text-sm font-bold ${uiPrefs.theme === t.id ? 'text-brand-700' : 'text-txt-main'}`}
                      >
                        {t.name}
                      </span>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {[
                    ...
                    >
                      <div
                        className={`text-xs font-bold ${uiPrefs.style === s.id ? 'text-brand-700' : 'text-txt-main'}`}
                      >
                        {s.name}
                      </div>
                    </button>
            ...
            <div className="space-y-3">
              <label className="text-sm font-bold text-txt-main block uppercase tracking-wider text-xs">
                Ambiance colorimétrique (Palette)
              </label>
              <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-9 gap-3">
                {[
                  ...
                  >
                    ...
                    />
                    <span
                      className={`text-xs font-bold uppercase tracking-tight text-center ${uiPrefs.colorPalette === p.id ? 'text-brand-700' : 'text-txt-muted'}`}
                    >
                      {p.name}
                    </span>
                  </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xs font-mono bg-canvas px-2 py-1 rounded text-txt-secondary">
                    {uiPrefs.fontSize}px
                  </span>
                </div>
                <input
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    >
                      <div
                        className={`text-sm font-bold ${f.fontClass} truncate ${uiPrefs.fontFamily === f.id ? 'text-brand-700' : 'text-txt-main'}`}
                      >
                        {f.name}
                      </div>
                  ...
                  {[
                    {
                      id: 'ultra',
                      name: 'Mode Expert',
                      desc: 'Densité maximale pour TCD massifs',
                      size: 10,
                      sw: 160
                    },
                    {
                      id: 'compact',
                      name: 'Mode Compact',
                      desc: 'Équilibre productivité/lisibilité',
                      size: 12,
                      sw: 192
                    },
                    {
                      id: 'comfortable',
                      name: 'Mode Confort',
                      desc: 'Lisibilité aérée et fluide',
                      size: 14,
                      sw: 240
                    }
                  ].map((p) => (
                    ...
                    >
                      <div
                        className={`p-1.5 rounded-md ${uiPrefs.density === p.id ? 'bg-brand-600 text-white' : 'bg-canvas text-txt-muted'}`}
                      >
                        <Maximize2 className="w-3.5 h-3.5" />
                      </div>
                      <div className="flex-1">
                        <div
                          className={`text-xs font-bold ${uiPrefs.density === p.id ? 'text-brand-700' : 'text-txt-main'}`}
                        >
                          {p.name}
                        </div>
            ...
            <div className="flex-1 space-y-6 pt-2 w-full max-w-lg">
              <div className="bg-brand-50 border border-brand-100 rounded-lg p-4">
                ...
                <p className="text-xs text-brand-800 mb-4 leading-relaxed">
                  Sélectionnez une image pour remplacer le titre "DataScope". Nous recommandons un
                  fichier <strong>PNG avec fond transparent</strong> pour un meilleur rendu.
                </p>
