...
const WidgetDisplay: React.FC<{ widget: DashboardWidget, data: any }> = ({ widget, data }) => {
   ...
   if (!data) return <div className="flex items-center justify-center h-full text-txt-muted text-xs">Chargement...</div>;
   if (data.error) return <div className="flex items-center justify-center h-full text-danger-text text-xs text-center p-2">{data.error}</div>;
   ...
   if (widget.type === 'text') {
      ...
      return <div className={`h-full w-full p-2 overflow-y-auto custom-scrollbar whitespace-pre-wrap ${size} ${color}`} style={{ textAlign: alignment }}>{widget.config.textContent || '...'}</div>;
   }
      ...
      return (
         <div className="flex flex-col h-full justify-center">
            <div className="flex items-end gap-2 mb-2">
               <span className="text-3xl font-bold text-txt-main">{current.toLocaleString(undefined, { maximumFractionDigits: 1 })}</span>
               <span className="text-sm text-txt-muted mb-1 font-medium">{unit}</span>
            </div>
            {style === 'progress' && target ? (
               ...
               <div className={`flex items-center text-xs font-medium ${isPositive ? 'text-success-text' : 'text-danger-text'}`}>
      ...
      return (
         <div className="h-full overflow-y-auto custom-scrollbar pr-2 space-y-3">
            {current.map((item: any, idx: number) => (
               <div key={idx} className="flex flex-col gap-1 cursor-pointer group" onClick={() => widget.config.dimension && setDashboardFilter(widget.config.dimension, item.name)}>
            ...
            <div className="flex items-center gap-2 border-r border-slate-100 pr-4 mr-2">
                ...
                <div className="flex items-center gap-2">
                    <div className="flex flex-col">
                        ...
                        <input 
                            type="date" 
                            className="text-xs border border-slate-200 rounded p-1 text-slate-700 bg-white focus:ring-blue-500 focus:border-blue-500"
