...
const FullscreenWidgetView: React.FC<{ widget: DashboardWidget; globalDateRange: any }> = ({
  widget,
  globalDateRange
}) => {
...
export const Dashboard: React.FC = () => {
  ...
  const openEditWidget = (w: DashboardWidget) => {
    ...
    let updatedWidget = {
      ...w,
      style: w.style || { borderColor: 'border-slate-200', borderWidth: '1' }
    };
  ...
  const secondaryFields = useMemo(() => {
    ...
    return (
      datasets.find((d) => d.id === tempWidget.config?.secondarySource?.datasetId)?.fields || []
    );
      ...
      {fullscreenWidgetId && fullscreenWidget && (
        <div className="fixed inset-0 z-50 bg-white p-8 flex flex-col animate-in zoom-in-95 duration-200">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-2xl font-bold text-slate-800">{fullscreenWidget.title}</h2>
            <button
